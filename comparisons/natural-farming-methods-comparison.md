---

title: Natural Farming Methods Comparison
created: 2026-04-11
updated: 2026-04-12
type: comparison

tags:
- comparison
- knf
- jadam
- microbes
- methods
- permaculture
- theory
- agriculture
- culture-media
- lighting
- farming

sources:
- raw/papers/chos-global-natural-farming.md
- raw/papers/permaculture-beginners-guide.md
- raw/papers/one-straw-revolution-masanobu-fukuoka.md
- raw/papers/administrator-microsoft-word-fukuoka-textdoc.md
---

# Natural Farming Methods Comparison

A comparison of the major natural farming approaches that avoid synthetic chemical inputs while maintaining or increasing productivity.

## Overview

| Method | Founder | Country | Era | Primary Focus |
|--------|---------|---------|-----|---------------|
| Fukuoka Natural Farming | Masanobu Fukuoka | Japan | 1940s-2008 | Do-nothing farming |
| Korean Natural Farming (KNF) | Cho Han-Kyu | Korea | 1960s-2018 | Fermented inputs, IMO |
| JADAM | Cho Youngsang | Korea | 1990s-present | Ultra-low-cost KNF |
| Biodynamic Agriculture | Rudolf Steiner | Germany | 1924-present | Cosmic rhythms, preparations |
| Permaculture | Mollison & Holmgren | Australia | 1978-present | Design systems, ecology |
| Nutrient-Dense Gardening | Steve Solomon | USA | 2000s-present | Complete mineralization |
| Regenerative Agriculture | Multiple | Global | 2010s-present | Soil health, carbon |

## Detailed Comparison

### Philosophy

| | Fukuoka | KNF | JADAM | Permaculture | Biodynamic | Solomon |
|---|---------|-----|-------|-------------|-----------|---------|
| Core idea | Nature knows best | Work with microbes | Simplest possible inputs | Design like ecosystems | Spiritual agriculture | Complete minerals |
| Approach | Minimalist | Systematic | Pragmatic | Comprehensive | Ritualistic | Scientific |
| Spirituality | Deeply spiritual | Some philosophical | Practical | Ethics-based | Central | None |

### Inputs

| | Fukuoka | KNF | JADAM | Permaculture | Biodynamic | Solomon |
|---|---------|-----|-------|-------------|-----------|---------|
| External inputs | None | Many fermented inputs | JMS + surfactant | Site-dependent | 9 preparations | Mineral mixes |
| Cost | Very low | Low-medium | Very low | Variable | Medium | Low |
| Local sourcing | Complete | Complete | Complete | Encouraged | Partial | Not required |
| Fermentation | None | Central | Used | Optional | Central | None |

### Soil Management

| | Fukuoka | KNF | JADAM | Permaculture | Biodynamic | Solomon |
|---|---------|-----|-------|-------------|-----------|---------|
| Tillage | No-till | Minimal | Minimal | Usually no-till | Varies | Optional |
| Fertilizer | Clover mulch ball | IMO + FPJ + OHN | JMS + WSF | Compost + mulch | BD compost + prep | COF mineral mix |
| Compost | Minimal | Not primary | Not primary | Central | Special methods | Supporting |
| Cover crops | White clover | Optional | Optional | Central | Used | Central |
| Microbes | Natural succession | Cultivated IMO | Cultivated JMS | Encouraged | BD preps | Indirect |

### Complexity & Learning Curve

| Method | Learning Curve | Number of Techniques | Time to Mastery |
|--------|---------------|---------------------|----------------|
| Fukuoka | Very high (philosophical) | Few techniques | Years of observation |
| KNF | Medium-high | 15+ input types | 1-2 seasons |
| JADAM | Low-medium | 3-5 core techniques | 1 season |
| Permaculture | High (design-intensive) | Many principles | 2-3 years |
| Biodynamic | High | 9 preparations + calendar | 2+ years |
| Solomon COF | Low | Soil test + 1 recipe | 1 season |

### Best Suited For

- **Fukuoka** — Grain farming, philosophical growers, patient observers
- **KNF** — Market gardeners, cannabis growers, systematic thinkers
- **JADAM** — Budget-conscious growers, large-scale farmers transitioning from chemical
- **Permaculture** — Homesteaders, landscape designers, long-term food systems
- **Biodynamic** — Wine growers, spiritual farmers, biodynamic certification seekers
- **Solomon COF** — Vegetable gardeners focused on nutritional quality

## Key Similarities

All natural farming methods share:

1. **Avoid synthetic chemicals** — No synthetic NPK, pesticides, or herbicides
2. **Feed the soil** — Healthy soil grows healthy plants
3. **Biology over chemistry** — Microbes are essential partners
4. **Local adaptation** — Methods must be adapted to local conditions
5. **Closed-loop thinking** — Minimize waste, maximize cycling
6. **Observation** — Watch and learn from natural systems

## Key Differences

1. **Input philosophy** — Fukuoka uses nothing; KNF uses many fermented inputs; Solomon uses mineral mixes
2. **Spirituality** — Biodynamic and Fukuoka are deeply spiritual; JADAM and Solomon are practical/scientific
3. **Scale** — Permaculture designs at landscape scale; KNF/JADAM work at field scale; Fukuoka at farm scale
4. **Certification** — Biodynamic and organic have certification paths; KNF and JADAM don't
5. **Animal integration** — Permaculture and biodynamic integrate animals; KNF focuses on plants; Solomon is plant-focused

## Choosing a Method

These approaches are not mutually exclusive. Many practitioners combine elements:

- **Permaculture design + KNF inputs** — common combination
- **Fukuoka philosophy + Solomon minerals** — minimalist with complete nutrition
- **JADAM cost savings + permaculture layout** — practical and well-designed
- **Biodynamic preps + regenerative grazing** — soil health + livestock integration

## See Also

- [[natural-farming-fukuoka]] — Fukuoka's method
- [[knf-preparations]] — KNF input catalog
- [[jadam-methods]] — JADAM simplified approach
- [[nutrient-dense-gardening]] — Solomon's mineral approach
- [[permaculture-design]] — permaculture design framework
- [[regenerative-agriculture-history]] — regenerative agriculture overview

## Related

- [[what-natural-farming-method-should-i-use]]
- [[youngsang-cho]]
