...
Some pathogens, notably *Pseudomonas syringae*, produce the JA-mimic **coronatine**, which hijacks the COI1-JAZ system to suppress SA-mediated defenses and create a favorable infection environment.

## The Salicylic Acid (SA) Pathway

### Biosynthesis and Core Mechanism

SA is synthesized primarily via the **isochorismate pathway** (involving ICS1/SID2) in chloroplasts, with a minor contribution from the phenylalanine ammonia-lyase (PAL) route. SA accumulation is triggered by pathogen-associated molecular patterns (PAMPs) and effector recognition.

The central regulatory hub is **NPR1 (Nonexpressor of PR Genes 1)**:

1. In uninfected plants, NPR1 exists as an oligomer in the cytoplasm, stabilized by disulfide bonds.
2. SA accumulation triggers redox changes (increased glutathione) that reduce NPR1 to its monomeric form.
3. Monomeric NPR1 translocates to the nucleus, where it interacts with TGA transcription factors to activate **pathogenesis-related (PR) protein** genes.
4. Key PR proteins include PR-1 (antifungal), PR-2 (β-1,3-glucanase), and PR-5 (thaumatin-like proteins).
...
The JA and SA pathways are **mutually antagonistic**, mediated by several molecular nodes:

| Mechanism | Direction | Effect |
|---|---|---|
| **NPR1–MYC2 interaction** | SA → suppresses JA | SA-activated NPR1 represses MYC2 transcription factor activity |
| **WRKY transcription factors** | Bidirectional | WRKY50/70 repress JA responses; WRKY33 modulates SA |
| **Mitogen-activated protein kinases (MAPKs)** | Context-dependent | MPK4 negatively regulates SA; MKS1 modulates both pathways |
...
| Feature | Jasmonic Acid (JA) | Salicylic Acid (SA) |
|---|---|---|
| **Primary threats** | Herbivores, necrotrophic fungi, wounding | Biotrophic pathogens, some viruses |
| **Bioactive form** | Jasmonoyl-isoleucine (JA-Ile) | Salicylic acid (SA), MeSA |
