---
title: "comparison companion planting reference vs jadam monoculture critique vs Monoculture"
created: 2026-04-28
tags: [comparison, planting-strategy, permaculture, organic-gardening, soil-health]
date: 2026-04-25
updated: 2026-04-25
sources:
  sources: []
type: comparison
---
# Companion Planting vs Monoculture
## Overview
Companion planting and monoculture represent opposite ends of the biodiversity
spectrum in food production. Companion planting arranges different species
together to create mutual benefits including pest deterrence, nutrient sharing,
improved pollination, and efficient space utilization. Monoculture grows a
single crop across a large area, maximizing efficiency of planting, maintenance,
and harvest but creating vulnerability to pests, disease, and soil depletion.
The tension between these approaches reflects a fundamental question in
agriculture: does biological diversity increase or decrease overall productivity
and system resilience?
## Companion Planting
[[jadam-monoculture-critique]]
JADAM, founded by Cho Han-kyu, challenges the widely accepted principle that
repeated monoculture necessarily degrades soil and requires crop rotation. The
core argument begins with a simple observation from nature: forests and
grasslands grow the same species in the same place for hundreds or thousands of
years without soil degradation, because mineral cycling is complete. Leaves,
branches, roots, and fruits all return to the soil to be disintegrated by
microbes, and the minerals they contain are released back into the biological
cycle. The deeper roots reach, the more mineral supply is brought to the
surface. That is how forests become greener and healthier over time.
The problem with human monoculture, according to JADAM, is not the
single-species planting itself but the removal of crop residues that breaks the
[[savory-mineral-cycle-management]]. After harvesting fruits, humans typically eliminate all remaining
crop residues (stems, leaves, roots), effectively exporting the soil's mineral
wealth with each harvest. The [[jadam-nutritional-balance-fertilizer]] of crop residue is perfectly
suited to the same crop that produced it, making it the ideal fertilizer. JADAM
argues that the conventional recommendation to destroy all crop residues because
they harbor disease organisms is misguided, since these pathogens are ubiquitous
in soil and impossible to eradicate. Instead, [[cervantes-pest-disease-prevention-overview]] should focus on
maintaining microbial diversity in the soil so that no single pathogen can
establish dominance.
JADAM draws an analogy with athlete's foot: people who walk barefoot never
develop athlete's foot because their feet are colonized by a diversity of
microorganisms that prevent any single fungus from dominating. The same
principle applies to farming: when soil hosts a rich diversity of bacteria,
fungi, and other microorganisms, no single pathogen can gain the upper hand.
JADAM has documented numerous case studies on its website where farmers resolved
supposed "[[jadam-monoculture-disorder-crop-residues]]" in peppers, strawberries, and fruit trees simply
by returning crop residues to the soil instead of removing and burning them.
## Key Differences
| Aspect | Companion Planting | Monoculture (JADAM approach) |
|--------|-------------------|------------------------------|
| Biodiversity | High (multiple species interplanted) | Low (single species) |
| Pest management | Biological (diverse plant defenses) | Microbial diversity in soil |
| [[comparison-nutrient-cycling-vs-nutrient-dense-gardening]] | Shared between species (N fixation) | Closed loop via residue return |
| Planning complexity | High (combinations, timing, spacing) | Low (single crop management) |
| Scalability | Garden to small diversified farm | Any scale |
| Equipment efficiency | Lower (different crops, different needs) | High (uniform crop, uniform treatment) |
| Harvest efficiency | Lower (staggered timing, mixed crops) | High (single harvest window) |
| Disease resistance | High (spatial diversity, trap crops) | Moderate (microbial competition) |
| Soil health approach | Diverse root exudates, N fixation | Residue return, microbial diversity |
| Yield per individual crop | Lower per crop, higher total system | Higher per crop |
| Knowledge required | High (plant interactions, families) | Moderate (soil biology, residue management) |
| Labor distribution | Spread across multiple tasks | Focused on single crop |
## When to Choose Companion Planting
Companion planting is the best approach for home gardens, permaculture food
forests, and small diversified farms where maximizing total production from
limited space matters more than harvesting efficiency. It is ideal for organic
growers who rely on biological pest management rather than chemical inputs.
Companion planting creates a more resilient system where the failure of one crop
does not devastate the entire garden, and the diversity supports pollinators,
predatory insects, and soil biology. Beginner gardeners benefit because
companion planting provides insurance against crop failure: if one species
struggles, others compensate.
## When to Choose Monoculture
Monoculture with JADAM's residue-return approach is practical when you need to
maximize efficiency for a single high-value crop, when equipment and
infrastructure are designed for one crop type, or when market demand focuses on
a specific product. JADAM's approach makes monoculture viable without the
traditional soil degradation, provided crop residues are systematically returned
to the soil and chemical inputs are minimized. This approach suits commercial
growers who need planting, maintenance, and harvest efficiency, and who can
manage soil biology through residue return and microbial diversity maintenance
rather than complex crop rotation schedules.
## Hybrid Approaches
The most resilient farming systems combine elements of both approaches. Use
monoculture beds for staple crops with full residue return and JADAM-style
microbial management, while interplanting companion species on bed edges, in
buffer zones, and along field margins. A JADAM-style monoculture pepper field
could have marigold borders for nematode control, basil interplanted at row
ends, and a diverse hedgerow of insectary plants (yarrow, dill, fennel) around
the perimeter. This maintains the equipment and harvesting efficiency of
## See Also

- [[comparison-companion-planting-vs-green-manure]]
- [[comparison-monoculture-vs-polyculture]]
- [[comparison-companion-planting-vs-polyculture]]
- [[comparison-polyculture-vs-intercropping]]
- [[comparison-living-mulch-vs-cover-crops]]
