---
title: Six-Row Seeder
source: eliot-coleman-winter-harvest-handbook.md
type: entity
tags: [tool, seeding, greenhouse, precision-agriculture]
---

# Six-Row Seeder

## Description
The six-row seeder is an improved [[coleman-precision-seeding-greenhouse-pinpoint-six-row-seeder]] implement that succeeded the original German-made pinpoint seeder used on Eliot Coleman's farm. Developed collaboratively after the German manufacturer discontinued the pinpoint seeder in 2004, the six-row version addressed design limitations found in the original while increasing capacity from four to six rows per pass. It plants twelve rows on a 30-inch bed in just two passes (compared to three passes with the four-row model).

## Classification
- **Category**: Agricultural tool / Precision seeder
- **Type**: Multi-row, walk-behind push seeder
- **Scale**: 30-inch bed width, 6-row configuration
- **Origin**: Collaboratively designed; manufactured after 2004

## Key Facts

### Development History
- Original German four-row pinpoint seeder discontinued by manufacturer in 2004
- Coleman organized a collaboration with other interested parties to find a new manufacturer
- New manufacturer engaged to produce a six-row version with improved features
- Designed to address features found wanting in the original pinpoint seeder
- Sources listed in Appendix C of *The [[winter-harvest-handbook-guide]]*

### Design Improvements Over Pinpoint Seeder
- Increased from four rows to six rows per pass
- Addresses specific ergonomic and functional shortcomings of the original
- Maintains the precision and accuracy that made the pinpoint seeder valuable
- Compatible with same bed preparation system (30-inch beds, firm soil surface)

### Performance
- Plants twelve rows on a 30-inch bed in two passes (vs. three passes for the four-row)
- Row spacing: 2.5 inches between rows — standard European greenhouse density
- Uses toothed wheels for consistent depth and spacing control
- Requires firm, smooth bed surface prepared with mesh roller

### Current Status
- Both the pinpoint seeder and six-row seeder remain in use on Coleman's farm
- Most information from the pinpoint seeder applies to the six-row model
- The six-row is the primary seeder for new users seeking this tool

### Bed Preparation Requirements
Same as pinpoint seeder:
1. Clear old crop
2. Broadfork for deep aeration
3. Tilther to incorporate amendments (e.g., alfalfa meal)
4. Apply compost (surface-applied, mixed shallowly)
5. Rake smooth with 30-inch Austrian hay rake
6. Firm bed surface with 30-inch mesh-surfaced roller

## Relevance to Cultivation and Mycology

### Precision Distribution
- Six-row precision exemplifies the importance of even spatial distribution in intensive biological production
- [[contamination-prevention-in-mushroom-cultivation]], even spawn distribution across substrate follows the same principle
- Dense, uniform planting maximizes resource utilization — applicable to both crops and [[mycelial-colonization]]

### Production Efficiency
- The evolution from four-row to six-row demonstrates continuous improvement in production tools
- Reducing passes (3 to 2) saves time and labor — directly applicable to [[bulk-substrate-preparation]] workflows
- The collaborative development model (users driving manufacturer design) parallels open-source tool development in mushroom cultivation

### Economic Viability
- Precision seeding enables greenhouse economics to work by maximizing output per square foot
- Same principle applies to controlled-environment mushroom production where space utilization directly affects profitability
- The tool investment pays back through increased production density and labor efficiency

## Crops Commonly Sown
- Baby leaf salads (twelve rows per 30-inch bed)
- Radishes (twelve rows per bed)
- Carrots (twelve rows per bed)
- Spinach (dense [[economics-of-winter-greenhouse-production]])
- Any crop suited to 2.5-inch row spacing

## See Also
- [[pinpoint-seeder]] — the predecessor tool (four-row)
- [[tilther]] — used for bed preparation before seeding
- [[broadfork]] — first step in the bed preparation sequence
- [[eliot-coleman]] — driving force behind the six-row seeder development
