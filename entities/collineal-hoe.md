---
title: Collineal Hoe
source: eliot-coleman-winter-harvest-handbook.md
type: entity
---

## Collineal Hoe

## Description

The collineal hoe is a hand tool innovation developed on Eliot Coleman's farm for efficient weed control. Named for its defining feature -- the cutting edge of the blade aligned in line ("co-linear") with the handle -- it represents the philosophy that small, thoughtful modifications to garden tools can dramatically improve their performance and the comfort of the user.

## Classification

- **Category**: Tool / Technique
- **Domain**: Weed control, market gardening
- **Type**: Draw hoe, modified hand tool

## Key Facts

- Blade dimensions: 7 inches side to side, 1 inch front to back (narrow profile)
- Optimal blade-to-handle angle: 70 degrees (determined through iterative field testing, starting from 90 to 80 to 75 to 70)
- The cutting edge is aligned in the same plane as the handle, unlike conventional hoes where the blade sits at a large angle
- Sharpened like a chisel with the sharp edge next to the soil; sharpened by holding the handle upright and filing across the upper edge
- Created by cutting, bending, and filing standard hoes on a workbench, then field-testing repeatedly
- Named "collineal" because the blade edge is collinear with the handle

## Design Rationale

Conventional hoes typically have large blades at 90-degree angles to the handle, which causes wrist strain and reduces cutting efficiency. Coleman observed that even small reductions in angle (to 80 degrees) improved comfort. Through iterative prototyping with multiple testers, the team converged on 70 degrees as optimal. The narrow blade profile allows precise cultivation close to crop plants without damaging them.

## Relevance to Cultivation

The collineal hoe embodies a key principle for market gardeners: tools should be adapted to the task and the user, not accepted as-is from the manufacturer. Coleman encourages growers to modify tools using basic workshop equipment (hacksaw, file, drill, pliers, vise). This approach improves both efficiency and ergonomics, reducing the physical toll of market gardening work. The development process -- iterative field testing with multiple users -- is a model for how to refine any cultivation tool or technique.

## Source Context

Described in Eliot Coleman's "The Winter Harvest Handbook" in the weed control and tools section.

## See Also

- [[broadfork]]
- [[collinear-hoe]]
- [[deep-organic-farming]]
- [[eliot-coleman]]
- [[flame-weeding-and-cultivation]]

## Practical Applications

Collineal Hoe has several important applications in sustainable agriculture
and organic growing systems. Growers and researchers have developed
practical methods for implementation across diverse climate zones.
Understanding these applications helps integrate the concept into
broader farming and gardening practice.

## Historical Significance

The historical development of collineal hoe reflects broader trends
in agricultural science and ecological thinking. Early observations
and experiments laid groundwork that modern practitioners continue
to build upon, connecting traditional knowledge with contemporary
research and sustainable management approaches.

## Environmental Impact

Collineal Hoe plays a role in broader ecosystem dynamics and environmental
stewardship. Its effects on soil health, biodiversity, and resource
cycling are relevant to ecological farming systems. Understanding
these impacts supports more informed management decisions and
sustainable land use planning.

## Research Directions

Ongoing research continues to expand understanding of this topic,
with studies examining interactions between biological, chemical,
and physical factors. Current investigations focus on optimizing
practical applications while maintaining ecological balance.
Peer-reviewed findings contribute to evolving best practices.

## Economic Considerations

The economic aspects of this topic include direct costs, labor
requirements, and potential returns for growers and producers.
Cost-benefit analyses help determine optimal implementation
strategies. Scale of operation and regional market conditions
significantly influence economic viability.

## Integration with Other Systems

This concept integrates with multiple agricultural and ecological
systems, creating synergies when properly managed. Connections
to soil biology, water management, and crop planning enhance
overall system resilience. Holistic approaches maximize benefits
across interconnected farm operations.
