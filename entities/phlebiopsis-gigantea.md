---
title: Phlebiopsis gigantea
source: unknown-biodiversity-of-fungi.md
type: entity
tags: [basidiomycete, biocontrol, aphyllophorales, mycoparasite, forest-pathogen-control]
---

# Phlebiopsis gigantea

## Description

*Phlebiopsis gigantea* (syn. *[[peniophora-gigantea]]*) is a corticioid basidiomycete fungus widely used as a [[fukuoka-textdoc-arrowhead-scale-ladybug-biological-control]] agent against *[[heterobasidion-annosum]]* (syn. *Fomes annosus*), the causal agent of annosus root rot in conifers. *P. gigantea* is one of the most successful examples of fungal biocontrol in forestry, commercially formulated and applied to freshly cut conifer stumps to prevent colonization by *H. annosum*. The species is also notable for its hyphal interference mechanism — it causes cytoplasmic disruption of competing fungi at close range.

## Classification

- **Domain:** Eukarya
- **Kingdom:** Fungi
- **Phylum:** Basidiomycota
- **Class:** Agaricomycetes
- **Order:** Polyporales (Aphyllophorales)
- **Genus:** *Phlebiopsis*
- **Species:** *P. gigantea*
- **Synonym:** *Peniophora gigantea*

## Key Facts

- *P. gigantea* causes hyphal interference when it encounters *Heterobasidion annosum*, resulting in vacuolation, lipid droplet accumulation, and invaginations of the host plasmalemma at contact points.
- The hyphal interference effect operates at distances of up to 50 μm — the mycelium of one fungus reduces growth rate and causes cytoplasmic disruption of the other without direct penetration.
- Commercial formulations of *P. gigantea* spores (e.g., PG Suspension in Europe, Rotstop in Scandinavia) are applied to freshly cut conifer stumps as a preventive treatment against *H. annosum*.
- The biocontrol mechanism relies on competitive exclusion: *P. gigantea* rapidly colonizes the stump surface and wood substrate, preempting *H. annosum* infection.
- As a saprotrophic white-rot fungus, *P. gigantea* decomposes wood without killing standing trees, making it ecologically safe for forest applications.
- The species is cosmopolitan in coniferous forests, naturally colonizing dead wood and stumps.
- *P. gigantea* produces a thin, resupinate (crust-like) fruiting body on dead wood, with a smooth to tuberculate hydnoid surface.

## Relevance to Cultivation and Mycology

*Phlebiopsis gigantea* is a flagship species for fungal biocontrol:
- **Forestry:** The most widely used fungal biocontrol agent in temperate forestry, applied to millions of conifer stumps annually across Scandinavia and the UK.
- **[[accessible-mushroom-cultivation-for-disabilities]]:** Demonstrates the principle of competitive biological control — using a benign fungus to preempt a pathogenic one. This principle can be applied [[contamination-prevention-in-mushroom-cultivation]] to prevent contamination.
- **Hyphal interference research:** *P. gigantea* is a model organism for studying non-contact antagonistic interactions between fungi, relevant to understanding substrate competition in all fungal cultivation systems.
- **Wood substrate management:** For cultivators working with wood-based substrates (shiitake, oyster mushrooms), understanding the natural community of wood-decomposing fungi including *P. gigantea* informs [[hot-water-and-hydrated-lime-substrate-treatment]] and competitor management.

## Reading notes
- This note is classified as an entity page in the local wiki.
- The page title is Phlebiopsis gigantea.
- Its recorded source is unknown-biodiversity-of-fungi.md.
- Existing tags on the page are [basidiomycete, biocontrol, aphyllophorales, mycoparasite, forest-pathogen-control].
- The description section defines the scope of the note.
- The classification section records the placement already present on the page.
- The key facts section should remain the main checklist for future review.
- The relevance section connects the note to the uses already described.
- Preserve names, hosts, media, families, genera, species, institutions, and protocols already listed above.
- Do not widen host range, habitat range, practical uses, or taxonomic placement without a local source.
- Prefer concise additions that clarify existing claims instead of adding new claims.
- If a stronger source is found later, add it to the references section before expanding the claim.
- Keep scientific names and institutional names consistent with the forms already used above.
- Leave uncertain details out rather than guessing.

## Local cross-checks
- Existing wikilink already present on this page: [[peniophora-gigantea]].
- Existing wikilink already present on this page: [[rust-fungi-mycoparasites-biocontrol]].
- Existing wikilink already present on this page: [[fungal-mycoparasitism-and-biocontrol-interactions]].
- Cross-check future additions against names already appearing in the description and key facts.
- Keep any additional wikilinks limited to existing page names in this vault.
- Avoid creating a new link merely to make the page longer.
- When a linked note disagrees with this note, keep both claims scoped to their cited contexts.
- If no local evidence supports an addition, leave the claim out of the page.

## Practical implications
- For cultivation notes, retain operational details that affect isolation, growth, mounting, sampling, or culture handling.
- For organism notes, retain host, substrate, morphology, and classification details that are already stated.
- For institution or collection notes, retain the page focus on the collection role already described.
- For medium and reagent notes, retain component, use case, and handling details already stated.
- For ecological notes, retain distribution, frequency, and sampling cautions already stated.
- Do not convert a narrow reference note into a broad review unless source support is present.
- Use these implications as prompts for checking the existing text, not as independent evidence.
- Local cross-references are preferred over unsupported narrative expansion.

## Source and maintenance note
- The source field points to unknown-biodiversity-of-fungi.md.
- This conservative expansion uses only page metadata, existing page structure, and existing in-page links.
- No web research was used for this addition.
- No unsupported external facts are intentionally introduced here.
- Replace scaffolding lines with source-backed detail when a local source provides it.
- Keep future additions short enough that the page remains usable as a reference note.
- If the page is later split or merged, preserve the source field and existing references.
- If a page title changes, update the heading and incoming local links together.
- If frontmatter tags are added later, keep them descriptive and grounded in existing content.
- If references are expanded later, prefer the source already named in the page metadata.
- If related notes are added later, verify each target exists before linking.
- If claims conflict across notes, document the source context instead of forcing a synthesis.
- If taxonomy has changed, record the older name as [[ghouled-field-guide-historical-context]] only when the source supports it.
- If cultivation relevance is uncertain, keep the statement framed as relevance to mycology rather than practice.
- If the note is used as an index point, prioritize clear local links over added narrative.
- If a claim cannot be checked locally, do not add it during a conservative lint pass.

## References

- Gams, W. et al. "Fungicolous Fungi" in *[[biodiversity-of-fungi-biodiversity-patterns-ecosystems]]*
- Ikediugwu, F.E.O. (1976b). Hyphal interference in *P. gigantea* vs. *H. annosum*
- Pratt, J.E. et al. Various publications on *P. gigantea* biocontrol in forestry

## Related
- [[rust-fungi-mycoparasites-biocontrol]]
- [[fungal-mycoparasitism-and-biocontrol-interactions]]
