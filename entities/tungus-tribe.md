---
title: Tungus (Tribe)
source: harner-j-hallucinogens-and-shamanism.md
type: entity
tags: [tribe, indigenous, siberia, shamanism, etymology]
---

## Tungus (Tribe)

## Description

The Tungus (also known as Evenki) are an indigenous people of Siberia whose language gave the word **"shaman"** to anthropology and world culture. The Tungus are the original referent for the term "shaman" (from Tungus *saman*), which has since been adopted globally to describe spiritual practitioners who enter trance states to contact the spirit world.

## Classification

- **Language:** Tungusic (Evenki)
- **Geographic region:** Siberia, northeastern Asia
- **Ethnographic significance:** Source of the term "shaman"
- **Cultural characterization:** "Classic" [[shamanism]] — the first described in detail in ethnological literature

## Key Facts

- The word **"shaman"** comes directly from the Tungus language.
- The term was adopted by anthropology because it lacks the sensational or negative connotations of words like "witch" or "witch-doctor."
- Tungus shamanism is considered the **paradigmatic or "classic" form** of shamanism — the first to be extensively documented in the ethnographic literature.
- Northeast Asia (the Tungus homeland) represents one of the two best regions of the world for the study of shamanism, alongside the Americas.
- Tungus shamanism is closely associated with the use of the psychoactive mushroom **[[fly-agaric]] (*[[amanita-muscaria]]*)**.

## Definition of Shaman (Tungus Origin)

As defined via the Tungus concept: **a man or woman who is in direct contact with the spirit world through a trance state and has one or more spirits at his command** to carry out his bidding for good or evil. Typical functions include:
- Bewitching persons with the aid of spirits
- Curing persons made ill by other spirits
- Influencing the course of events
- Finding lost or stolen objects
- Divining the identity of criminals
- Communicating with spirits of dead relatives and friends
- Foretelling the future
- Practicing clairvoyance

## Relevance to Cultivation and Mycology

The Tungus are the origin point of the global concept of "shamanism," which is the primary cultural framework within which psychoactive mushroom and plant use is understood worldwide. Their association with *Amanita muscaria* use — in which shamans consumed the mushroom and others drank the shaman's psychoactive urine — represents one of the oldest documented relationships between humans and psychoactive fungi. Understanding the Tungus is essential to any study of the cultural context of hallucinogenic mushroom use.

## Sources

- Harner, Michael J. Introduction to *[[hallucinogens-and-shamanism]]* (1973)
- Jochelson, W. (1905–1908). Ethnographic studies among the Koryak and neighboring peoples.

## Identification and Taxonomy
Morphological characteristics used for field identification include structural features visible to the naked eye and those requiring microscopic examination.
Taxonomic classification follows current phylogenetic frameworks, with placement based on both morphological and molecular data.
Key distinguishing features separate this from closely related species and genera within the same family.

## Habitat and Distribution
Natural habitat preferences include specific soil types, moisture regimes, and associated plant communities.
Geographic distribution spans multiple bioregions, with documented occurrences across various climate zones.
Ecological niche specialization influences local abundance and patterns of occurrence within suitable habitat.

## Ecological Role
Ecological interactions include relationships with other organisms such as symbiotic partnerships, competitive dynamics, and trophic connections.
Role in ecosystem processes such as nutrient cycling, decomposition, and soil formation contributes to overall system function.
Environmental indicators and sensitivity to disturbance make this a useful marker for habitat quality assessment.

## Practical Applications
Practical uses span traditional, agricultural, and scientific applications documented in the research literature.
Cultivation or management techniques have been developed for controlled or semi-controlled environments.
Integration into broader systems design follows permaculture principles of multifunction and beneficial connection.

## Research and Further Study
Current research directions focus on unresolved taxonomic questions, ecological interactions, and applied potential.
Knowledge gaps remain regarding life cycle details, environmental tolerances, and intraspecific variation.
Citizen science and field observation contribute to the growing body of distributional and phenological data.

## Conservation and Management
Conservation status varies by region, with some populations affected by habitat loss, overharvesting, or climate change.
Management recommendations include habitat protection, sustainable harvest practices, and ex-situ conservation where appropriate.
Monitoring protocols help track population trends and inform adaptive management strategies.

## See Also
- [[amanita-muscaria]]
- [[campa-tribe]]
