---
title: "Pleurotin"
created: 2026-04-28
type: chemical
tags:
  - sesquiterpene
  - fungal-metabolite
  - antibiotic
  - anticancer
  - pleurotus
date: 2026-04-28
updated: 2026-04-28
sources:
  - "Hervey, A. & Brian, P.W. (1948). \"Studies on the production of antibiotics by fungi.\" Annals of Applied Biology, 35(4), 542-552."
  - "Buu-Hoï, N.P. et al. (1955). \"Pleurotin, a new antibiotic pigment.\" Journal of the Chemical Society, 813-816."
  - "Evidente, A. et al. (1998). \"Pleurotin and analogues: fungal metabolites with antibiotic and phytotoxic activity.\" Phytochemistry, 48(3), 517-523."
  - "Fischer, E. et al. (2014). "Isolation and structure elucidation of pleurotin-type sesquiterpenes." Journal of Natural Products, 77(9), 2075-2080."
---

# Pleurotin

## Overview

**Pleurotin** is a yellow crystalline sesquiterpene quinone antibiotic produced by the
[[singh-basidiomycete-fungi-in-petroleum-hydrocarbon-cleanup]].

First isolated in 1948 from *Pleurotus griseus* cultures, pleurotin was among the
earliest fungal-derived antibiotics discovered and represents a structurally unique
spiro-cyclic sesquiterpene scaffold.

The compound exhibits notable antibacterial activity against Gram-positive bacteria
and has attracted renewed attention for its selective cytotoxicity against cancer cell
lines, particularly through inhibition of the thioredoxin-thioredoxin reductase system.

## Chemical Properties

- **Molecular formula:** C₁₅H₁₆O₄
- **Molecular weight:** 260.29 g/mol
- **IUPAC name:** (1R,5aS,9aS,9bR)-1,5a,6,9a,9b,10-Hexahydro-5,5a-dimethyl-9b-(2-oxopropyl)-3H-2,8-dioxacyclopenta[a]pentalene-4,7-dione
- **Appearance:** Yellow crystalline solid
- **Melting point:** 178–180 °C (decomposes)
- **Solubility:** Soluble in chloroform, ethyl acetate, and acetone; poorly soluble in water

Pleurotin belongs to the **illudinane** class of sesquiterpenes, characterized by a
spiro[cyclopropane-1,1'-indane] ring system fused to a quinone moiety.

The spiro-cyclopropane ring is a key pharmacophore responsible for its biological
activity, acting as a Michael acceptor that forms covalent adducts with nucleophilic
thiol groups in target proteins.

## Pharmacology

Pleurotin's primary mechanism of action is **inhibition of thioredoxin reductase (TrxR)**,
a selenocysteine-containing flavoenzyme critical for maintaining intracellular redox
homeostasis.

- **IC₅₀ against purified TrxR:** ~0.3–1.2 µM (species-dependent)
- **Selectivity index:** Cancer cells show 5–15× greater sensitivity than non-malignant
  cells, attributed to elevated [[staphylococcus-aureus]]*, *[[reactive-oxygen-species-and-oxidative-stress]] and apoptotic cell death.

This mechanism overlaps with other TrxR inhibitors (e.g., auranofin) but pleurotin's
fungal-derived scaffold offers a distinct chemical space for drug development.

## Biosynthesis and Source Organisms

Pleurotin is biosynthesized via the **farnesyl diphosphate (FPP) pathway**, with a
class II sesquiterpene cyclase catalyzing the initial cyclization to form the
illudinane skeleton, followed by oxidative tailoring enzymes.

**Known producing organisms:**
- *Pleurotus griseus* (original source, type strain)
- *[[pleurotus-cornucopiae]]*
- *Clitocybe* species (reported in screening programs)

Fermentation studies on *P. griseus* demonstrate peak pleurotin production at 25 °C
in static liquid culture after 14–21 days, with yields of 5–15 mg/L in unoptimized
conditions.

Genomic mining has identified the pleurotin biosynthetic gene cluster, enabling
heterologous expression studies in *[[aspergillus-oryzae]]* that increased titers to
~50 mg/L.

## Toxicity and Safety
