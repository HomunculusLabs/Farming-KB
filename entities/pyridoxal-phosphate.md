---
title: Pyridoxal Phosphate
type: chemical
aliases:
  - PLP
  - P5P
  - pyridoxal 5'-phosphate
  - pyridoxal-5-phosphate
  - pyridoxal phosphate
  - vitamin B6 phosphate
chemical_class: vitamin B6 vitamer coenzyme
formula: C8H10NO6P
molecular_weight: 247.14 g/mol
pubchem_cid: 1051
inchi_key: NGVDGCNFYWLIFO-UHFFFAOYSA-N
created: 2026-05-02
updated: 2026-05-02
related_concepts:
  - pyridoxal-phosphate-coenzyme-chemistry
sources:
  - https://pubchem.ncbi.nlm.nih.gov/compound/1051
  - https://en.wikipedia.org/wiki/Pyridoxal_phosphate
---

# Pyridoxal Phosphate

Pyridoxal phosphate is the main catalytically active coenzyme form of vitamin B6.
It is usually abbreviated PLP in biochemistry and enzymology literature.
The name pyridoxal 5'-phosphate emphasizes that the phosphate is attached to the 5-hydroxymethyl substituent of pyridoxal.
PLP is a small organic cofactor rather than a protein or mineral nutrient.
Its formula is C8H10NO6P, and its PubChem compound identifier is CID 1051.
Its neutral molecular weight is about 247.14 g/mol.
PubChem lists the IUPAC name as (4-formyl-5-hydroxy-6-methyl-3-pyridinyl)methyl dihydrogen phosphate.
The InChIKey is NGVDGCNFYWLIFO-UHFFFAOYSA-N.
PLP belongs to the family of vitamin B6 vitamers, which also includes pyridoxine, pyridoxal, pyridoxamine, and their phosphate forms.
In organisms, these vitamers can be interconverted through salvage and phosphorylation reactions.

## Structural Features

PLP contains a substituted pyridine ring.
The ring nitrogen can participate in electron withdrawal when protonated in an enzyme active site.
A phenolic hydroxyl group sits on the ring and contributes to hydrogen bonding and tautomeric chemistry.
A methyl group marks the vitamin B6 scaffold and helps distinguish PLP from other pyridine cofactors.
The aldehyde group is the reactive handle that forms imines with amino groups.
The phosphate ester is not usually the main catalytic center, but it strongly anchors PLP in enzymes.
The phosphate group makes PLP more polar than free pyridoxal.
The phosphate also supplies binding energy through ionic interactions with arginine, lysine, serine, threonine, and backbone amides.
In many PLP enzymes, the cofactor is buried deeply enough that its reactivity is controlled by the surrounding protein.
The conjugated aldehyde-pyridine system is the basis of PLP's electron-sink behavior.

## Biological Role

PLP is best known as a coenzyme for amino acid metabolism.
It supports transamination, decarboxylation, racemization, beta-elimination, gamma-elimination, replacement, and one-carbon reactions.
The International Union of Biochemistry and Molecular Biology has catalogued more than one hundred PLP-dependent enzymatic activities.
A common estimate is that PLP-dependent activities account for roughly four percent of classified enzyme activities.
This breadth makes PLP one of the most versatile organic cofactors in metabolism.
Its central role follows from the chemical instability of many amino acid reaction intermediates.
PLP stabilizes carbanion-like character that would otherwise be too reactive in water.
The cofactor therefore lets enzymes break bonds next to an amino group under physiological conditions.
PLP enzymes are central to nitrogen redistribution, neurotransmitter biosynthesis, sulfur amino acid metabolism, and heme synthesis.
They are also important in microbial specialized metabolism and natural product biosynthesis.

## Binding in Enzymes

In a resting PLP-dependent enzyme, the aldehyde commonly forms a Schiff base with an active-site lysine.
This enzyme-bound imine is called the internal aldimine.
Substrate entry replaces the lysine imine with an imine between PLP and the substrate amino group.
That substrate-bound form is called the external aldimine.
The external aldimine is the branch point for many PLP reaction mechanisms.
The protein positions the substrate so that the bond to be broken is aligned with the PLP pi system.
This stereoelectronic rule is often described through the Dunathan hypothesis.
By orienting different bonds, homologous PLP enzymes can catalyze different reaction classes.
The cofactor supplies general chemical power, while the enzyme active site supplies selectivity.
Small differences in active-site acid-base residues can change whether the outcome is transamination or decarboxylation.

## Catalytic Chemistry

The aldehyde carbon of PLP is electrophilic toward amino groups.
The imine linkage makes the substrate's alpha carbon electronically connected to the pyridine ring.
When a proton or substituent is removed, negative charge can delocalize into the cofactor.
Mechanistic drawings often represent this delocalized state as a quinonoid intermediate.
The quinonoid is not a free carbanion; it is an enzyme-bound resonance-stabilized species.
PLP can then help return electrons in a controlled direction to form product.
In transamination, PLP accepts an amino group and becomes [[pyridoxamine-phosphate]].
In the second half-reaction, pyridoxamine phosphate donates the amino group to a keto acid and regenerates PLP.
In decarboxylation, PLP stabilizes the intermediate formed after carbon dioxide leaves an amino acid.
In elimination reactions, PLP helps remove groups from beta or gamma positions relative to the amino group.

## Metabolic Examples

Alanine aminotransferase uses PLP to connect alanine and pyruvate metabolism.
Aspartate aminotransferase uses PLP to connect aspartate and oxaloacetate metabolism.
Glutamate decarboxylase uses PLP to produce gamma-aminobutyric acid, or GABA.
Aromatic L-amino acid decarboxylase uses PLP in dopamine and serotonin biosynthesis pathways.
Histidine decarboxylase uses PLP to make histamine.
Serine hydroxymethyltransferase uses PLP in one-carbon metabolism linked to folate chemistry.
Cystathionine beta-synthase and cystathionine gamma-lyase use PLP in sulfur amino acid metabolism.
Glycogen phosphorylase contains PLP, although in that enzyme the phosphate group has a distinctive catalytic role.
Delta-aminolevulinate synthase uses PLP in the first committed step of heme biosynthesis.
Microbial PLP enzymes also generate nonprotein amino acids and secondary metabolites.

## Nutrition and Clinical Relevance

Humans require vitamin B6 from the diet because the B6 scaffold is an essential nutrient.
PLP is the functional form used by many enzymes after dietary vitamers are absorbed and converted.
Deficiency can impair neurotransmitter production, amino acid metabolism, and red blood cell formation.
Severe deficiency is associated with neurological symptoms, including seizures.
Certain drugs can reduce functional PLP availability by forming hydrazone-like adducts or otherwise disturbing vitamin B6 metabolism.
Isoniazid is a classic example discussed in relation to B6 supplementation.
Serum PLP is commonly used as a biomarker for vitamin B6 nutritional status.
Because PLP is enzyme-bound in many tissues, blood concentration is only one view of whole-body status.
High-dose supplementation should be distinguished from normal coenzyme function.
Excess vitamin B6 intake can cause toxicity, especially sensory neuropathy, even though PLP itself is essential.

## Research and Laboratory Context

PLP enzymes are common subjects in mechanistic enzymology because they give spectroscopic signatures.
Internal and external aldimines often have characteristic visible absorbance bands.
Quinonoid intermediates can sometimes be trapped or observed during rapid-reaction experiments.
Crystal structures often reveal PLP covalently bound to an active-site lysine.
Mutating the lysine or phosphate-binding residues typically changes activity dramatically.
PLP-dependent enzymes are also attractive targets for antimicrobial and antiparasitic drug discovery.
Inhibitors often mimic amino acid substrates or form stable adducts with the cofactor.
The high conservation of PLP chemistry creates both opportunities and selectivity challenges.
PLP is also relevant in synthetic biocatalysis, where transaminases are used to make chiral amines.
Engineered PLP enzymes can expand the range of amino acid and amine transformations available to industry.

## Connections

The concept page [[pyridoxal-phosphate-coenzyme-chemistry]] explains PLP as an electron-sink catalyst.
The entity [[pyridoxamine-phosphate]] is the aminated partner formed during transamination.
PLP connects vitamin nutrition to enzyme mechanism, amino acid metabolism, and neurochemistry.
It is an entity because it is a specific compound with a defined structure, identifiers, and biochemical role.
