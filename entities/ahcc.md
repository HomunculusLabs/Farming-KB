---
title: AHCC
type: chemical
tags:
  - supplement
  - mushroom-extract
  - immunomodulator
  - oligosaccharide
  - medicinal-mushroom
  - cancer-adjuvant
  - immune-support
  - basidiomycota
  - mycotherapy
  - hepatoprotective
date: 2026-05-09
updated: 2026-05-09
sources:
  - Ghoneum, M. (1995). "Augmentation of NK cell activity in cancer patients by ahcc-active-hexose-correlated-compound Compound (AHCC)." Anti-Cancer Drugs, 6(3), 434-438.
  - Matsui, Y., et al. (2002). "Improved method for the preparation of AHCC." Japanese Patent JP2002356801.
  - Ito, K., et al. (2014. "Effect of AHCC on immune response in healthy volunteers." Journal of [[fukuoka-critique-western-dietetics-nutritional-science]] and Vitaminology, 60(4), 262-268.
  - Cowawintaweewat, S., et al. (2016). "AHCC supplementation: a review of clinical evidence." Integrative Cancer Therapies, 15(1), 44-55.
  - https://www.ahccresearch.com/ (manufacturer research database)
aliases:
  - Active Hexose Correlated Compound
  - AHCC extract
  - Lentinula edodes mycelial extract
---

# AHCC (Active Hexose Correlated Compound)

**AHCC** (Active Hexose Correlated Compound) is a proprietary, standardized extract derived from the **mycelium** of several basidiomycete mushrooms, primarily *Lentinula edodes* (shiitake). Developed in Japan in the late 1980s, AHCC is one of the most clinically researched mushroom-derived immunomodulatory supplements, with over 30 human clinical trials and 100+ preclinical studies investigating its effects on immune function, cancer outcomes, liver health, and infectious disease. It is marketed as a dietary supplement in over 40 countries and is widely used in Japanese integrative oncology as a [[blesching-cannabis-beta-caryophyllene-cb2-spices-complementary-therapy]] alongside conventional cancer treatment.

## Composition and Manufacture

### Source Material
AHCC is produced from the mycelium of multiple basidiomycete mushroom species. The primary organism is *Lentinula edodes* (shiitake), though the exact proprietary blend includes additional species from the genera *Pleurotus* and *Trametes*. The mycelium is cultured in liquid [[cancer-medicinal-mushrooms-submerged-fermentation-production]] on a rice bran-based substrate under controlled temperature and pH conditions.

### Manufacturing Process
The production of AHCC involves several critical steps that distinguish it from conventional mushroom extracts:

1. **Liquid culture:** Mycelial fermentation in large-scale bioreactors (typically 5–10 days)
2. **Enzymatic digestion:** The harvested mycelial biomass is treated with proprietary enzyme preparations that partially hydrolyze cell wall polysaccharides
3. **Incubation:** The hydrolysate is incubated at 40–50°C for an extended period (typically 48–72 hours), during which further enzymatic reactions occur
4. **Concentration and sterilization:** The liquid extract is concentrated under reduced pressure and sterilized by ultrafiltration
5. **Spray drying:** The concentrated extract is spray-dried to produce a light brown, water-soluble powder

This unique manufacturing process breaks down high-molecular-weight β-glucans into partially acetylated **oligosaccharides** of approximately 5 kDa average molecular weight, which is the defining characteristic of AHCC.

### Chemical Composition
AHCC is a complex mixture of [[cordyceps-cancer-bioactive-compounds]]:

- **Partially acetylated α-1,4-glucans:** The principal active components (estimated 30–40% of dry weight), with an average molecular weight of ~5,000 Da
- **β-glucan oligosaccharides:** Lower-molecular-weight β-linked glucose polymers
- **Amino acids:** Including glutamine, arginine, and branched-chain amino acids
- **Minerals:** Calcium, magnesium, zinc, and selenium
- **Other components:** Trace amounts of chitin, nucleic acids, and [[aact-tannins-terpenes-phenolic-compounds-wood-composting-ingham]]

The acetylated α-glucan fraction is unique to AHCC and not found in standard mushroom β-glucan preparations. This distinction is believed to underlie AHCC's distinct immunological profile compared to other mushroom-derived immunomodulators.

## Mechanism of Action

### Immune Cell Activation
AHCC exerts broad immunomodulatory effects through activation and enhancement of multiple immune cell populations:

- **Natural Killer (NK) cells:** AHCC significantly increases NK cell activity and cytotoxicity within days of supplementation. NK cell activation is mediated through upregulation of perforin and granzyme B expression and enhanced target cell recognition
- **Dendritic cells (DCs):** AHCC promotes DC maturation, migration, and antigen-presenting capacity. Mature DCs show increased expression of MHC class II, CD80, CD86, and CD40 co-stimulatory molecules
- **T lymphocytes:** AHCC enhances CD4⁺ and CD8⁺ T cell proliferation, increases Th1 cytokine production (IL-2, IFN-γ), and promotes cytotoxic T lymphocyte (CTL) activity against tumor-associated antigens
- **Macrophages:** Activation of peritoneal and alveolar macrophages with enhanced phagocytic activity and cytokine secretion
- **B lymphocytes:** Modest enhancement of antibody production and B cell proliferation

### Cytokine Modulation
AHCC shifts the cytokine profile toward a **Th1-dominant** pattern, increasing IL-2, IL-12, IFN-γ, and TNF-α while modulating IL-6 and IL-10. This Th1 bias is considered therapeutically advantageous for anti-tumor immunity and antiviral defense, as Th1 responses drive cellular immunity against infected and malignant cells.

### Gut Immunity
AHCC influences the gut-immune axis by modulating intestinal microbiota composition and enhancing gut-associated lymphoid tissue (GALT) function. It increases secretory IgA production and strengthens intestinal barrier integrity, contributing to systemic immune surveillance.

## Clinical Evidence

### Cancer Adjunct Therapy
The largest body of clinical evidence for AHCC concerns its use as a complementary therapy in cancer patients. Key findings from controlled clinical trials include:

- **Hepatocellular carcinoma:** Post-surgical AHCC supplementation (3 g/day) significantly improved overall survival and recurrence-free survival in multiple Japanese trials, with 5-year survival rates improving from 50% to 70% in some studies
- **Gastric cancer:** Reduced postoperative complications and improved quality of life scores
- **Breast, colorectal, and pancreatic cancers:** Improved immune parameters, reduced chemotherapy side effects (fatigue, nausea, neutropenia), and enhanced treatment tolerance
- **Tumor markers:** Decreased levels of CEA, CA19-9, and AFP in various cancer types during AHCC supplementation

### Hepatoprotection
AHCC protects liver function through multiple mechanisms:

- Reduction of serum ALT, AST, and GGT in patients with chronic hepatitis
