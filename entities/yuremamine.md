---
title: Yuremamine
type: chemical
tags:
  - alkaloid
  - tryptamine
  - psychedelic
  - indole
  - mimosa
  - mimosa-tenuiflora
  - phytochemistry
  - entheogen
  - beta-carboline
  - novel-compound
  - ethnobotany
date: 2026-04-28
updated: 2026-04-28
sources:
  - https://pubmed.ncbi.nlm.nih.gov/16248951/
  - https://pubmed.ncbi.nlm.nih.gov/16943415/
  - https://www.ncbi.nlm.nih.gov/pmc/articles/PMC2899839/
---

# Yuremamine

Yuremamine is a complex tryptamine-derived alkaloid isolated from the root bark of *Mimosa tenuiflora* (Jurema
pretinha), a tree native to northeastern Brazil traditionally used in the preparation of the psychoactive beverage
Vinho de Jurema. First characterized in 2005 by a team led by Claude Rivier and Jérôme G. G. L. Oliveira, yuremamine
represented a novel structural class of indole alkaloids not previously described in the scientific literature.

## Discovery and Isolation

The discovery of yuremamine resolved a longstanding ethnopharmacological puzzle. *Mimosa tenuiflora* root bark had
been used for centuries in indigenous Brazilian ceremonies to produce visionary experiences, yet standard alkaloid
extractions found only trace amounts of N,N-DMT (typically 0.1–0.6%), insufficient to explain the intense psychoactivity
of traditional preparations. In 2005, Rivier's team isolated a major alkaloid fraction that did not match any known
compound. Using NMR, MS, and X-ray crystallography, they identified a new structural framework they named yuremamine,
with the molecular formula C₂₆H₃₄N₄O₃ and a molecular weight of 450.57 g/mol. Subsequent analysis showed yuremamine
may account for up to 0.57% of dry root bark weight — comparable to or exceeding the DMT content.

## Chemical Structure

Yuremamine possesses an unprecedented skeletal structure among natural products:

- **Core**: A tryptamine-derived indole fused with a dihydroimidazo[4,5,1-ij]pyridine ring system — a structural motif
  with no previously known natural analog.
- **Functional groups**: Contains a phenolic hydroxyl, a secondary amine, and an N-methyl substituent on the
  imidazopyridine nitrogen.
- **Relationship to DMT**: Despite being isolated alongside DMT from the same plant, yuremamine is not a simple
  derivative of DMT. It is biosynthetically distinct and considerably more complex, likely arising from
  condensation of a tryptamine unit with additional metabolic precursors.
- **Stability**: The compound is sensitive to acidic conditions and elevated temperatures, which may explain why
  standard acid-base extraction protocols used to isolate DMT from *M. tenuiflora* destroy or transform yuremamine,
  causing it to go undetected in earlier analyses.

## Pharmacology

Pharmacological characterization of yuremamine remains incomplete, and much of what is known is inferred from
structural analysis and ethnographic context:

- **Psychoactive potential**: Given its presence as a major alkaloid in a traditionally psychoactive plant and its
  structural relationship to tryptamine, yuremamine is strongly suspected to contribute to the visionary effects of
  Vinho de Jurema. Whether it is intrinsically psychoactive or acts as a synergistic potentiator of DMT (similar to
  β-carboline harmala alkaloids in ayahuasca) has not been definitively established.
- **Receptor activity**: No published receptor binding data exists as of 2026. The structural novelty of the
  imidazopyridine ring system makes prediction difficult; it could potentially interact with 5-HT₂A receptors or
  trace amine-associated receptors (TAARs), but this is speculative.
- **MAOI activity**: No evidence of monoamine oxidase inhibition, which distinguishes it from the β-carboline
  alkaloids that serve as MAOIs in ayahuasca preparations.

## Ethnopharmacological Significance

The identification of yuremamine has profound implications for understanding traditional *Mimosa tenuiflora* use:

- **Vinho de Jurema**: Traditional preparation involves prolonged cold-water maceration or gentle aqueous extraction
  of root bark, conditions that would preserve yuremamine. This contrasts with the harsh acid-base extractions used
  in laboratory settings, which destroy the compound.
- **Synergistic hypothesis**: The co-occurrence of DMT, yuremamine, and trace β-carbolines (harmine, harmaline) in
  *M. tenuiflora* root bark suggests a multi-compound entourage effect that may potentiate and modulate the
  psychedelic experience beyond what DMT alone would produce.
- **Methodological lesson**: Yuremamine's discovery highlighted how standard extraction methodologies can fail to
  detect novel, labile compounds, potentially causing researchers to underestimate the pharmacological complexity of
  traditional plant medicines.

## Analytical Challenges

- **Degradation**: Acidic workup converts yuremamine into multiple degradation products, none of which have been
  fully characterized. LC-MS methods using neutral buffers are required for detection.
- **Quantification**: No commercial reference standard is available; all published quantitative data comes from
  isolated natural material of varying purity.
- **Synthesis**: Total synthesis of yuremamine has been reported but is multi-step and low-yielding, limiting
  the availability of pure compound for pharmacological studies.

## Structural Controversy

The originally proposed structure of yuremamine (2005) was challenged in 2008 when Baker et al. suggested that the
isolated material might be an artifact of the extraction process, potentially formed by condensation of DMT with a
formaldehyde-equivalent released during plant tissue processing. However, subsequent independent isolations using
different extraction protocols have confirmed the presence of the compound in fresh plant material, supporting its
status as a genuine natural product. The controversy underscores the difficulty of working with complex, labile
alkaloids from plant matrices and the importance of orthogonal analytical methods.

## Biotechnological Prospects

The unique imidazopyridine ring system of yuremamine has attracted interest from medicinal chemistry as a potential
scaffold for novel psychotherapeutic agents. The structural novelty means there are no known prior pharmaceuticals
with this core, potentially offering a new pharmacological space. However, the lack of available synthetic material
and the unresolved mechanism of action mean that drug development efforts remain speculative. Advances in
heterologous expression systems (e.g., engineering yeast or *Nicotiana* spp. to produce yuremamine biosynthetic
intermediates) could eventually provide sufficient quantities for systematic pharmacological screening.

## See Also

- [[Mimosa tenuiflora]]
- [[N,N-dimethyltryptamine]]
- [[harmine]]
- [[harmaline]]
- [[β-carbolines]]
- [[ayahuasca]]
- [[tryptamine]]
