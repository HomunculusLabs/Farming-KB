---
title: Cannabis Delivery Methods Comparison
created: 2026-04-16
updated: 2026-04-16
type: concept
tags: [cannabis, cultivation]
sources:
  - "raw/papers/marijuana-horticulture-cervantes.md"
---
# Cannabis Delivery Methods Comparison

Cannabis may be administered through multiple routes, each with distinct pharmacokinetic profiles affecting onset time, duration, bioavailability, and therapeutic character. Selecting the appropriate delivery method is a critical component of effective medical cannabis therapy, often as important as strain and dose selection.

## Inhalation Methods

### Vaporization
- **Mechanism:** Heats plant material to a precise temperature that evaporates cannabinoids just below the burning point [[staycare-degradation-of-plant-cell-wall-polymers-by-fungi]] matter
- **Onset:** Minutes to seconds
- **Duration:** Shorter than other methods (2-4 hours)
- **Advantages:**
  - Eliminates inhalation of burned carbon products
  - Precise temperature control preserves terpenes
  - Rapid onset allows easy dose titration
  - Cleaner than smoking
- **Disadvantages:**
  - Shorter duration of effects
  - Equipment cost for quality vaporizers
  - Some patients find the sensation unfamiliar

### Smoking
- **Onset:** Minutes to seconds
- **Duration:** 2-4 hours
- **Advantages:** Rapid onset, easy dose titration, familiar method
- **Disadvantages:**
  - Inhalation of burned plant material and carbon products
  - Potential respiratory irritation
  - Destruction of some terpenes at combustion temperatures
  - Many patients prefer vaporizers to minimize respiratory risk

## Oral Methods

### Edibles (Cooked Foods)
- **Mechanism:** Absorbed through the gastrointestinal tract after processing by the liver
- **Onset:** 45 minutes to 1.5 hours
- **Duration:** Significantly longer than inhalation (4-8 hours)
- **Effects:** Noticeably different from inhalation (often more body-focused, more sedating)
- **Advantages:**
  - Long-lasting relief
  - No respiratory impact
  - Easy to integrate into meals
- **Disadvantages:**
  - Delayed onset makes dose titration difficult
  - Effects can be more intense/unpredictable
  - Liver converts THC to 11-hydroxy-THC (more psychoactive)
- **Preparation:** Usually added to recipes as herbed butter, infused oils, or tinctures

### Sublingual/Oromucosal Tinctures
- **Mechanism:** Absorbed through mucous membranes under the tongue or inside the cheek
- **Onset:** 15-45 minutes (faster than GI absorption)
- **Duration:** Moderate (3-5 hours)
- **Advantages:**
  - Faster onset than edibles
  - More predictable dosing than edibles
  - Precise measurement via dropper bottles
  - Bypasses first-pass liver metabolism (partially)
  - Easy to add to foods after cooking
- **Disadvantages:**
  - Taste can be bitter (especially alcohol-based)
  - Shorter duration than edibles
- **Dosing:** Start with 3-4 drops, wait at least one hour, gradually increase

### Raw Cannabis (Juice/Salad)
- **Mechanism:** Fresh raw cannabis contains CBD and THC in their acid forms (CBD-acid, THC-acid)
- **Key Feature:** THC-acid is only minutely psychoactive
- **Advantages:**
  - Non-psychoactive at therapeutic doses
  - High CBD-acid content for therapeutic and preventative use
  - Nutrient-dense when consumed as salad or juice
- **Disadvantages:**
  - Limited clinical research on raw cannabis
  - Bitter taste (often diluted with other vegetable juices)
  - Primarily physician [[stamets-mycoremediation-case-studies]] and patient testimonials available

### Suppositories
- **Mechanism:** Cannabis-infused oils inserted rectally, similar to a suppository
- **Onset:** Variable
- **Advantages:**
  - Delivers cannabinoids to the lower intestinal tract
  - Useful for gastrointestinal conditions
  - Avoids first-pass metabolism
  - No psychoactive effects (primarily local action)
- **Disadvantages:**
  - Unfamiliar/uncomfortable for many patients
  - Limited research on rectal bioavailability

## Topical Methods

### Topical Creams and Balms
- **Mechanism:** Applied directly to skin for localized relief
- **Onset:** 15-60 minutes
- **Duration:** Variable (2-6 hours)
- **Advantages:**
  - Delivers medicine to specific, isolated problem areas
  - No psychoactive effects (minimal systemic absorption)
  - Can be combined with other therapeutic herbs
  - Safe for localized pain, inflammation, skin conditions
- **Disadvantages:**
  - Absorption rates may be less systematic than oral methods
  - Effects are more local than systemic
- **Bases:** Hemp oil, almond oil, coconut oil (preferably virgin, organic)
- **Preparation:** Cannabis tinctures or oils mixed with topical bases; also combined with spice plants containing (E)-beta-caryophyllene

## Pharmaceutical Delivery Methods

### Sativex (Oromucosal Spray)
- Standardized whole-plant cannabis extract (THC:CBD)
- Each spray delivers 2.7mg THC and 2.5mg CBD
- Self-titrating spray mechanism
- Widely studied in clinical trials (especially for MS)

### [[dronabinol]] (Marinol)
- Synthetic THC in capsule form
- Oral administration with standard dosing
- Predictable but lacks full entourage effect

### [[nabilone]] (Cesamet)
- Synthetic cannabinoid in capsule form
- Primarily used as antiemetic

## Method Selection Guide

| Need | Recommended Method | Rationale |
|------|-------------------|-----------|
| Rapid onset | Vaporization | Seconds to minutes |
| Long-lasting relief | Edibles | 4-8 hours duration |
| Precise dosing | Sublingual tincture | Dropper measurement |
| Localized pain | Topical cream | Direct application |
| Avoid psychoactivity | Raw juice, topical | Non-psychoactive routes |
| GI tract targeting | Suppository | Local rectal delivery |
| Nighttime use | Edible or indica tincture | Long duration, sedating |
| Daytime use | Vaporization | Short duration, easy control |

## Safety Note

It is not safe to drive or operate heavy machinery after taking any psychoactive form of cannabis, regardless of delivery method.

## See Also
- [[blesching-cannabis-cannabinoid-terpene-entourage-effect]]
- [[reactive-metabolites-and-bioactivation-toxicology]]
- [[first-pass-metabolism-and-oral-bioavailability]]
