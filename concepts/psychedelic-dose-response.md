---
title: Psychedelic Dose-Response
created: 2026-04-13
updated: 2026-04-13
type: concept
tags: [psychedelics, pharmacology, dosage, harm-reduction, threshold, synergy, set-and-setting, dose-response-curve]
sources: [raw/papers/the-essential-psychedelics-guide-by-dm-turner.md]
---

## Psychedelic Dose-Response Relationships

Understanding the dose-response relationship is fundamental to working with psychedelic compounds safely and effectively. D.M. Turner's Essential Psychedelics Guide, combined with Shulgin's TIHKAL data (see [[tihkal-introduction-and-methodology]]), provides a practical framework.

### The Shulgin Scale

Alexander Shulgin developed a practical intensity scale used throughout TIHKAL:

- **Minus** (-): No effect or below threshold
- **Plus one** (+1): Very mild effect, threshold; slight alterations in perception or mood
- **Plus two** (+2): Clearly noticeable effects; enhanced colors, mild visual changes, mood alteration
- **Plus three** (+3): Strong effects; significant visual and perceptual changes, difficulty with ordinary tasks, emotional intensity
- **Plus four** (+4): Overwhelming experience; ego dissolution, complete reality replacement, mystical peak experience, loss of contact with ordinary consciousness

### Non-Linear Amplification

Psychedelics do not follow a simple linear dose-response curve. The relationship is markedly non-linear:

- Doubling the dose does not simply double the intensity — it can produce qualitatively different experiences
- The jump from +2 to +3 is often larger than from +1 to +2
- The jump from +3 to +4 is the most dramatic: it is not merely "more intense" but categorically different — ego death, complete reality dissolution, encounters with autonomous entities

This is why [[psychedelic-experience-stages]] change character so dramatically across dose ranges.

### Compound-Specific Dose Ranges

**Psilocybin** (pure chemical, from [[psilocin-and-psilocybin-chemistry]]):
- Light (+1/+2): 5-10 mg
- Moderate (+2/+3): 10-20 mg
- Strong (+3): 15-30 mg
- Very strong (+4): 30+ mg
- Mushroom equivalent (Psilocybe cubensis): Light 0.5-1.5 g, Moderate 1.5-3.5 g, Strong 3.5-5+ g

**DMT** (see [[dmt-compound-profile]]):
- Smoked: 20-60 mg (+1 to +3), 60-100 mg (+3 to +4)
- Intravenous: 4-15 mg (threshold to +3), 30 mg (+4)
- Oral: inactive without MAOI (see [[ayahuasca-pharmacology]])

**5-MeO-DMT** (see [[5-meo-dmt-compound-profile]]):
- Smoked: 6-10 mg (+2), 10-20 mg (+3 to +4)
- Intravenous: 0.5-2 mg (+1 to +3)

### Threshold Effects

Every compound has a threshold dose below which no subjective effects are detectable. For DMT (i.v.), this is approximately 4 mg. Below this, the compound cannot be distinguished from placebo. Just above threshold (8 mg), physical effects appear without mental effects. The gap between physical and mental thresholds is significant and compound-dependent.

### Synergy Effects

**Compounds potentiating each other**:
- MAO inhibitors + tryptamines (the ayahuasca principle): allows oral activity and extends duration
- Cannabis + psychedelics: commonly reported as dramatically intensifying the experience (sometimes described as "multiplying" rather than "adding")
- [[leary-psychedelic-manual-approach|Leary]] described synergy as one of the key variables in controlling psychedelic experience intensity

**Compounds attenuating each other**:
- Benzodiazepines can reduce psychedelic intensity but may not eliminate it
- Antipsychotics (especially 5-HT2A antagonists) can terminate the experience

### Set and Setting as Dose Multipliers

Turner emphasizes that psychological and environmental factors act as effective dose multipliers:
- Fearful or resistant mindset: can amplify difficult aspects
- Supportive environment, trusted companions: can smooth the experience
- Familiarity with the compound: experienced users often need less for equivalent subjective intensity
- Music: can dramatically shape the character and direction of the experience

### Tolerance

Classic psychedelics (tryptamines, lysergamides, phenethylamines) show rapid tolerance development with cross-tolerance between compounds acting on the same receptors. Tolerance typically builds within hours and resolves within 3-7 days of abstinence. Notably, DMT is an exception — Shulgin's research showed no tolerance development with repeated i.v. doses at half-hour intervals.

## See Also

- [[psychedelic-experience-stages]]
- [[tihkal-introduction-and-methodology]]
- [[dmt-compound-profile]]
- [[psilocin-and-psilocybin-chemistry]]
- [[ayahuasca-pharmacology]]
- [[leary-psychedelic-manual-approach]]

Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.
Field observations and practical experience remain the most reliable guides for system optimization.