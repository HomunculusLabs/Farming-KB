---

title: Fermented Mixed Compost (FMC) — IMO-Enriched Compost
created: 2026-04-11
updated: 2026-04-12
type: concept

tags:
- knf
- compost
- fermentation
- soil
- recipe
- organic
- processing
- cover-crop

sources:
- raw/papers/chos-global-natural-farming.md
- raw/papers/master-cho-knf-recipe-book.md
- raw/papers/beneficial-indigenous-microorganisms-bionutrients.md
---

# Fermented Mixed Compost (FMC)

Fermented Mixed Compost is KNF's signature composting method. It combines IMO-4 (cultivated indigenous microorganisms) with organic matter and select KNF inputs to create a microbially-rich, nutrient-balanced compost. Unlike traditional composting which relies on ambient microbes, FMC actively inoculates the compost with targeted beneficial microorganisms from your local environment.

FMC is the bridge between KNF liquid inputs and long-term soil building. While KNF sprays provide immediate nutrition, FMC builds the soil food web infrastructure that sustains plant health between applications.

## Two Methods

Cho describes two approaches to FMC: a simpler field method and a detailed precision method.

### Method 1: Simple Field FMC

A straightforward approach for on-farm compost production.

**Materials (per 1000 kg organic matter)**:
- Organic matter (crop residue, animal manure, leaf mold): 1000 kg
- [[indigenous-microorganisms-imo]] IMO-4: 200-300 kg
- Water: 200-300 Liters (adjust for moisture)
- Optional: FPJ, FAA, OHN, LAB diluted 1:1000 as moisture source

**Process**:
1. Prepare organic matter — chop crop residues into 5-10 cm pieces for faster decomposition
2. Mix organic matter with IMO-4 thoroughly on soil floor (not concrete)
3. Adjust moisture to 60-65% (squeeze test: holds shape when squeezed, breaks when dropped)
4. Build pile on soil floor — maximum height 50-70 cm
5. Cover with rice straw or tarp to retain moisture and heat
6. Turn pile every 3-5 days for first 2 weeks
7. After 2 weeks, turn every 7 days
8. Compost is ready in 4-6 weeks when:
   - Temperature drops to ambient
   - Material is dark, crumbly, and earthy-smelling
   - Original materials no longer recognizable
   - White mycelium present (beneficial fungi)

### Method 2: Detailed Precision FMC

Cho's comprehensive method with precise layering and nutrient input scheduling.

**Materials (per 1000 kg organic matter)**:

| Ingredient | Amount | Function |
|-----------|--------|----------|
| Organic matter base | 1000 kg | Carbon source, bulk |
| Animal manure (cow/pig/chicken) | 200-500 kg | Nitrogen source |
| Rice bran | 50-100 kg | Microbial food, carbon |
| Charcoal (powdered) | 50-100 kg | Porosity, microbial habitat, toxin absorption |
| IMO-4 | 200-300 kg | Microbial inoculation |
| Sea soil (soil from coastal areas) | 50-100 kg | Trace minerals |
| Red clay/loess | 50-100 kg | Minerals, microbial habitat |
| FPJ diluted 1:1000 | 20-30 L | Microbial energy |
| FAA diluted 1:1000 | 10-20 L | Nitrogen boost |
| OHN diluted 1:1000 | 10-20 L | Disease prevention |
| LAB diluted 1:1000 | 10-20 L | Lactic acid bacteria boost |
| Seawater | 3-5 L | Full-spectrum minerals |
| Water (non-chlorinated) | As needed | Moisture adjustment |

**Process**:

1. **Site preparation**: Build on soil floor (NOT concrete). Shade from direct sun. Protect from heavy rain.

2. **Layering** (build pile in layers, not all mixed at once):
   - Base layer: 10 cm of coarse organic matter (stalks, branches)
   - Second layer: 10 cm of fine organic matter + manure
   - Third layer: Sprinkle rice bran, charcoal, sea soil, red clay
   - Fourth layer: Spray diluted FPJ, FAA, OHN, LAB, seawater
   - Fifth layer: IMO-4 (crumble and distribute evenly)
   - Repeat layers until pile reaches 50-70 cm height
   - Top layer: Cover with rice straw or gunny sack

3. **Temperature management** (critical):
   - Monitor pile temperature daily
   - Ideal range: 40-60C (104-140F)
   - Below 40C: Too wet or too cold → turn and add dry material
   - Above 60C: Thermophilic phase — beneficial but don't let it exceed 70C
   - Above 70C: Nutrients are lost, beneficial microbes die → turn immediately
   - Target: 3 days above 55C for pathogen kill, then maintain 45-55C

4. **Turning schedule**:
   - Day 3-5: First turn (when temp reaches 50-60C)
   - Day 7-10: Second turn
   - Day 14: Third turn
   - Day 21: Fourth turn
   - Day 28: Fifth turn (if needed)
   - After each turn: re-cover with straw

5. **Moisture management**:
   - Check moisture at each turn
   - Target: 60-65% throughout process
   - If too dry: spray diluted NF inputs (FPJ, LAB at 1:1000)
   - If too wet: add dry rice bran or charcoal, turn more frequently

6. **Curing** (after active composting):
   - When temperature drops to ambient and stays there for 3+ days, active composting is complete
   - Let compost cure for 2-4 weeks in covered pile
   - Curing allows microbial communities to stabilize and phytotoxic compounds to break down

**Total time**: 6-8 weeks from start to finished compost

### Signs of Quality FMC

- Dark brown to black color
- Earthy, pleasant smell (no ammonia, no putrid odor)
- Crumbly texture — breaks apart easily
- Original materials not recognizable
- White fungal mycelium visible when broken open
- Moist but not wet
- Temperature at ambient

### Storage

- Store in covered pile, protected from rain and direct sun
- Use within 3-6 months for maximum microbial activity
- Can be stored longer but microbial diversity declines over time
- Keep covered with breathable material (jute, straw, not plastic)

## Seasonal Timing

Make FMC between November and March for best results. Cool, dry weather favors fermenting microbes (Aspergillus oryzae, Absida, Mucor, Rhizopus) over bacteria. In summer's heat and humidity, bacteria multiply too fast and outcompete fermenters, causing decay. The first stage of FMC is saccharification — fermenting microbes make sugars unimpeded by bacteria in late fall/winter conditions.

## Material Selection Philosophy

From Cho's recipe book — four principles for choosing FMC ingredients:

1. **Include all four realms**: mountain (fallen leaf or fruit), paddy (rice bran or straw), field (oil dreg, bean dreg, bush clover), and sea (fish waste or seaweed)
2. **Protein-rich animal materials**: bone meal, fish meal as major ingredients alongside plant materials
3. **Avoid agricultural by-products** (chemically treated residues)
4. **Microbe food = human food = plant food**: choose high-quality diverse materials
## See Also

- [[knf-fermented-mixed-compost]]
- [[cho-fermented-mixed-compost-methods]]
