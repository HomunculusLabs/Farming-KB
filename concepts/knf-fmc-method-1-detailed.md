---
title: "KNF Fermented Mixed Compost Method-1 Detailed"
created: 2026-04-20
updated: 2026-04-20
type: concept
tags: [knf, natural-farming]
sources:
  - raw/papers/cho-han-kyu-korean-natural-farming-handbook.md
---

# KNF Fermented Mixed Compost Method-1 Detailed

**Updated:** 2026-04-18

Fermented Mixed Compost (FMC) is the primary soil amendment in Korean Natural Farming. Method-1 is the detailed, temperature-monitored composting process that produces high-quality humus for soil building. Unlike conventional composting, FMC Method-1 follows a strict turning schedule based on internal temperature readings, with the goal of completing fermentation in approximately 20 days while maintaining sweet-smelling, aerobic conditions.

## Overview

FMC is made from a mixture of organic materials fermented with [[knf-fermented-mixed-compost]] for materials, timing, and place selection.

## Temperature Control: The Critical Factor

The success of FMC Method-1 depends entirely on temperature management:

- **Target fermentation range:** 40-50C
- **Turning trigger:** Turn when temperature reaches 50C
- **Maximum temperature:** NEVER allow temperature to exceed 70C
- **Danger zone:** Above 70C, beneficial microbes die and the compost quality deteriorates

### Why Temperature Matters

At temperatures below 40C: fermentation is too slow, unwanted microbes may dominate
At 40-50C: optimal zone for beneficial thermophilic microorganisms
At 50C: time to turn the pile (heat buildup indicates oxygen depletion)
At 60C: many beneficial organisms begin to die
At 70C: sterilization occurs -- the compost becomes biologically dead
Above 70C: irreversible damage, potential fire hazard

## The Turning Schedule

The pile must be turned at specific times based on temperature readings:

### Day 2: First Turn

- By day 2, the pile should reach 50C from initial microbial activity
- Turn the entire pile thoroughly, moving material from outside to inside and bottom to top
- This reintroduces oxygen and redistributes moisture and microbes
- After turning, temperature will drop temporarily, then rise again

### Days 5-6: Second Turn

- The pile reaches 50C again approximately 3-4 days after the first turn
- Turn thoroughly as before
- Check moisture: the pile should feel like a wrung-out sponge
- If too dry, add water while turning; if too wet, add dry material

### Days 8-10: Third Turn

- The pile reaches 50C again approximately 2-4 days after the second turn
- This is typically the last required turn
- After this turn, the fermentation rate slows

### Days 15-20: Completion

- Temperature should gradually decline
- By day 20, fermentation is essentially complete
- The compost should have a sweet, earthy smell
- If the compost smells sour, rotten, or like ammonia, something went wrong

## Signs of Success and Failure

### Successful FMC (Sweet Smell)

- Sweet, earthy aroma (like forest floor after rain)
- Dark brown to black color
- Original materials are no longer recognizable
- Texture is crumbly and uniform
- Temperature has stabilized near ambient
- No offensive odors

### Failed FMC (Indicators)

- **Sour smell:** Anaerobic conditions -- too wet or too compact, needed earlier turning
- **Ammonia smell:** Too much nitrogen-rich material, insufficient carbon, or incomplete fermentation
- **Putrid/rotten smell:** Anaerobic decomposition by wrong organisms -- pile may need to be restarted
- **Dry and powdery:** Insufficient moisture -- add water during next turning
- **Still hot after 20 days:** Fermentation not complete -- may need additional turns

## Application of FMC

### How to Apply

- Apply before sunset (reduces UV damage to microbes)
- Apply in lump form (not spread thinly)
- Cover with soil or mulch after application (protects microbes from sun and drying)
- Apply to the root zone of target plants

### Timing

- Best applied before planting or during early growth stages
- Can be applied as a top dressing during the growing season
- Avoid applying during heavy rain (nutrient loss through runoff)
- Can be applied to established orchard trees at any time except during dormancy

### Quantity

- Standard rate: 1-2 tons per 100 ares (1 hectare) for normal soil
- Degraded soil: 3-4 tons per 100 ares
- For individual trees: 5-10 kg per tree depending on tree size
- For vegetable beds: 1-2 kg per square meter

## Relationship to IMO Progression

FMC Method-1 typically uses [[knf-fish-amino-acid]]: Adds nitrogen and amino acids (use sparingly to avoid ammonia odor)
- [[lactic-acid-bacteria-knf]]: Accelerates fermentation and suppresses pathogens
- [[knf-yeast-preparation-detailed]]: Completes the fermentation process
## See Also
