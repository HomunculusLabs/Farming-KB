...
- The equation is part of [[maxwell-equations-electromagnetism]], not an isolated rule for coils alone.
- It remains valid in vacuum, dielectrics, conductors, and magnetic materials, though constitutive laws change the response.

## Eddy currents

- Eddy currents are circulating currents induced inside bulk conductors exposed to changing magnetic fields.
- They dissipate energy as heat and create opposing magnetic fields according to Lenz law.
- Induction heating deliberately uses eddy-current and hysteresis losses to heat conductive workpieces without direct electrical contact.
- Eddy-current brakes slow moving conductors without friction, useful in trains, exercise machines, amusement rides, and test rigs.
- Nondestructive testing measures eddy-current response to find cracks, thickness changes, alloy differences, and heat-treatment variations.
- Transformer and motor cores are laminated or made from ferrites to break large current loops and reduce losses.
- Skin effect at high frequency is related: induced fields push alternating current toward conductor surfaces.
- Shielding can redirect or absorb changing magnetic fields, but shield conductivity, thickness, permeability, and frequency all matter.

## Circuit models

- Inductance summarizes flux linkage per unit current in a circuit element or geometry.
- For a linear inductor, voltage equals L times rate of current change, and stored energy equals one half L I squared.
- Mutual inductance describes how current change in one circuit induces voltage in another circuit.
- Dot convention records winding polarity so circuit equations preserve the physical Lenz-law sign.
- Real inductors include winding resistance, core loss, parasitic capacitance, saturation, and thermal limits.
- Fast switching circuits can create damaging induced voltages unless flyback diodes, snubbers, clamps, or controlled layouts are used.
- Twisted pairs reduce unwanted loop area and therefore reduce pickup from changing magnetic flux.
- Ground loops can behave as accidental induction loops in instrumentation systems.
...
- Magnetic materials introduce saturation, hysteresis, remanence, permeability variation, and frequency-dependent losses.
- Copper windings introduce resistance, temperature rise, insulation stress, skin effect, proximity effect, and mechanical forces during faults.
- High voltage induction requires insulation coordination because winding-to-winding and turn-to-turn stresses can exceed simple average values.
- High frequency designs may be limited more by parasitics and core losses than by the ideal Faraday law.
- Thermal design is essential because induction devices often fail through insulation aging caused by heat.
- Electromagnetic compatibility requires controlling stray fields, loop areas, return paths, shielding, and switching edges.
- Measurement circuits must distinguish true induced emf from capacitive pickup, contact potentials, and ground-reference artifacts.
- Safe design accounts for stored magnetic energy, arc risk, implanted medical devices, and strong-field mechanical attraction.

## Misconceptions

- A steady magnetic field through a stationary closed loop does not by itself produce transformer emf.
- The induced current opposes the flux change, not necessarily the magnetic field itself.
- Faraday law is not only about wires; it is a statement about electric fields in space.
- The negative sign is essential because it represents back action and energy conservation.
- A larger magnetic field does not guarantee larger induced voltage unless flux is changing more rapidly.
- Voltage across an open coil can exist even when no current flows because the circuit is incomplete.
- Iron cores are not required for induction, but they greatly increase flux linkage in many devices.
- Ideal transformer equations are approximations that ignore leakage, losses, saturation, and bandwidth limits.
