---
title: Juncao Technology for accessible-mushroom-cultivation-for-disabilities
tags: [juncao, mushroom, substrate, grass, cultivation, china, pleurotus, alternative-substrate, sustainable]
date: 2026-04-28
updated: 2026-04-28
sources:
  - raw/papers/unknown-oyster-mushroom-cultivation.md
created: 2026-04-28
type: concept
---

# Juncao Technology for Mushroom Cultivation

Juncao (菌草) is a Chinese term where Jun (菌) means fungi and Cao (草) means grass. Juncao technology represents a comprehensive approach to cultivating edible and medicinal fungi using herbaceous plants as [[growing-gourmet-substrate-materials-wood-types]], replacing traditional substrates such as sawdust, wheat bran, and rice bran.

## Origins and Development

Juncao techniques were invented in 1983 by Professor Zhanxi Lin, director of Fujian Agriculture and Forestry University and the Juncao Research Institute in China. Before Juncao technology, many species of edible and medicinal fungi, including shiitake and wood's ear, were primarily cultivated on sawdust or logs. The expansion of mushroom production led to over-exploitation of broad-leaf tree resources, causing a shortage of raw [[mushworld-oyster-substrate-materials-worldwide-survey]]. Major mushroom-producing countries like Japan and China encountered these problems, which restricted further large-scale production development.

The invention of Juncao techniques resolved the conflict between increasing fungi production and protecting ecological environments, enabling fast and [[sustainable-mushroom-cultivation]] production growth.

## Definitions

Three formal definitions structure the Juncao framework:

- **Juncao**: Herbaceous plants suitable for cultivating edible and medicinal fungi.
- **Juncao Technology**: A series of comprehensive techniques to cultivate edible and medicinal fungi and produce mycelium protein forage using Juncao grasses.
- **Juncao Industry**: The industry utilizing Juncao technology and related techniques.

## Solar Energy Conversion Advantage

The conversion rate of solar energy into Juncao grasses is 6 to 8 times higher than that for broad-leaf trees. With Juncao techniques, approximately 1 kg of dry grass can be converted into about 1 kg of fresh mushrooms. This technology combines ecological, economical, and social benefits. The Juncao Industry represents a new ecological industry with high natural resource utilization rates and great potential for continual development.

## Nutritional Superiority Over Sawdust

Test results from the Fujian Agriculture University Central Laboratory demonstrate that nutrient contents of most Juncao species are richer than those of sawdust. The contents of protein, nitrogen, fat, phosphorus, potassium, and magnesium in Juncao are higher than in broad-leaf tree sawdust. Among the tested species:

- Protein content of wild Dicranopteris dichotoma, Neyraudia reynaudiana, Saccharum arundinaceum, Phragmites communis, Miscanthus floridulus, and Themeda gigantea is 2 to 4 times that of sawdust.
- Fat content ranges from 101 to 216% of sawdust levels.
- Nitrogen content ranges from 232 to 353% of sawdust levels.
- Phosphorus content ranges from 225 to 685% of sawdust levels.
- Potassium content ranges from 346 to 908% of sawdust levels.
- Magnesium content ranges from 191 to 303% of sawdust levels.

### Nutritional Comparison Table

| Species | Protein (%) | Fiber (%) | Fat (%) | N (%) | P (%) | K (%) | Ca (%) | Mg (%) |
|---------|------------|-----------|---------|-------|-------|-------|--------|--------|
| Sawdust (reference) | 1.19 | 84.82 | 0.93 | 0.19 | 0.02 | 0.11 | 0.22 | 0.03 |
| Dicranopteris dichotoma | 3.75 | 72.10 | 2.01 | 0.60 | 0.09 | 0.37 | 0.22 | 0.08 |
| Neyraudia reynaudiana | 4.42 | 58.80 | 1.72 | 0.67 | 0.14 | 0.96 | - | 0.09 |
| Saccharum arundinaceum | 2.75 | 62.50 | 0.99 | - | - | 0.76 | 0.17 | 0.09 |
| Phragmites communis | 3.19 | 72.50 | 0.94 | 0.51 | 0.08 | 0.85 | 0.14 | 0.06 |
| Miscanthus floridulus | 3.56 | 55.10 | 1.44 | 0.57 | 0.08 | 0.90 | 0.30 | 0.10 |
| Themeda gigantea | 3.85 | 51.10 | 1.38 | 0.61 | 0.05 | 0.72 | 0.19 | 0.08 |
| Pennisetum purpureum | 5.91 | 68.88 | - | - | 0.18 | 0.78 | 0.40 | 0.24 |
| [[spartina-alterniflora]] | 9.90 | 23.58 | 2.96 | - | - | - | - | - |
| Sorghum propinquum | 4.17 | 49.47 | - | - | 0.08 | 0.46 | 0.44 | 0.17 |

## Selected Juncao Species

From 1983 to 2003, 37 Juncao species were selected as suitable for mushroom cultivation using a three-stage selection method. These include:

### Grass Species

- Pennisetum purpureum (napier grass)
- Pennisetum sinense
- [[pennisetum-alopecuroides]]
- Phragmites communis (common reed)
- Neyraudia reynaudiana
- Miscanthus floridulus
- Miscanthus sacchariflorus
- [[miscanthus-sinensis]] (Chinese silver grass)
- Saccharum arundinaceum
- Saccharum sinense
- Saccharum robustum
- Themeda gigantea
- Paspalum wettsteinii
- Paspalum dilatatum
- Vetiveria zizanioides ([[comparison-vetiver-grass-vs-bamboo-for-erosion-control]])
- Sorghum propinquum
- Sorghum sudanense
- [[cymbopogon-citratus]] (lemongrass)
- Arundo donax (giant reed)
- Arundinella hirta
- Arundinella nepalensis
- Spartina anglica
- Achnatherum splendens

### Non-Grass Species
