...
Chamovitz's *What a Plant Knows* explores how plants possess an internal sense of direction,
a biological equivalent of proprioception, allowing them to grow roots downward and shoots
upward even when displaced from their original position.
...
that some signal traveled from the cap to the elongation zone to produce bending. They
performed increasingly precise ablation experiments to demonstrate that even minute damage
to the cap impaired gravitropism, establishing the root cap as a sensory organ in its own
right.

## Statocytes and Statoliths

Gravity sensing in plants relies on specialized cells called **statocytes**, located in the
**columella** of the root cap and in the **endodermis** of shoots. Within these cells sit
**amyloplasts** — starch-filled organelles also known as **statoliths**. Because statoliths
are denser than the surrounding cytoplasm, they settle to the bottom of the cell under the
pull of gravity, much like a stone sinking in water.

When a plant organ is reoriented, statoliths shift to the new "bottom" of the statocyte.
This displacement is detected by the cell — likely through pressure on the endoplasmic
...
Plant organs respond to gravity in opposite directions depending on their identity:
...
The **Cholodny-Went model** provides the canonical explanation for how auxin redistribution
produces gravitropic bending. Proposed independently by Nikolai Cholodny and refined by
Fritz Went, the model states that when a plant organ is placed horizontally:

1. **In roots**, auxin accumulates on the **lower side**. In roots, high auxin
   concentrations **inhibit** cell elongation. The upper side elongates while the lower
   side is suppressed, causing the root to bend **downward**.
2. **In shoots**, auxin also accumulates on the **lower side**. In shoots, high auxin
   concentrations **stimulate** cell elongation. The lower side elongates more, causing
   the shoot to bend **upward**.

This explains how a single hormone drives opposite bending directions in different organs:
the organs simply respond differently to the same concentration gradient. The model has
been refined over decades but its core — lateral auxin redistribution underlies gravitropic
bending — remains well supported by direct measurements using radiolabeled IAA and
auxin-responsive reporter genes such as DR5.
...
Plants rarely experience a single directional cue in isolation. In nature, **gravitropism**
and **phototropism** operate simultaneously, and plants must integrate these signals.
Phototropic cues typically dominate in shoots when light is available, ensuring stems grow
