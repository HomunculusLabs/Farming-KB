---
title: Singh Laccase-Mediated Biodegradation of PAHs and Dyes
created: 2026-04-21
updated: 2026-04-21
type: concept
tags: [mycoremediation, biology, pollution, microbes]
sources: [raw/papers/singh-harbhajan_-mycoremediation-_-fungal-bioremediation.md]
---

# Singh Laccase-Mediated Biodegradation of mycoremediation-of-pahs|PAHs and Dyes
#mycoremediation #laccase #pah #dye-decolorization #enzymology #biodegradation

Updated: 2026-04-18

Source: Harbhajan Singh, Mycoremediation: [[singh-ligninolytic-enzymes-fungal-bioremediation]], [[lignin-degradation-mechanisms-wood-rot-fungi]] among fungi. It catalyzes the one-electron oxidation of phenolic substrates to phenoxy radicals while reducing molecular oxygen to water. The enzyme contains four copper atoms at its active site. Without mediators, laccase has a limited redox potential that restricts it to phenolic substrates. With mediators, it can oxidize nonphenolic compounds with higher redox potentials.

## Key Mediators

### HBT (1-hydroxybenzotriazole)
- Most effective mediator for mycoremediation-of-pahs|PAH degradation
- Enables oxidation of nonphenolic mycoremediation-of-pahs|PAHs by laccase
- Dramatic enhancement: from 3-37% to 91-100% degradation

### ABTS (2,2'-azino-bis(3-ethylbenzothiazoline-6-sulfonic acid))
- Enables very rapid degradation
- Coriolopsis gallica laccase + ABTS: 100% anthracene in 10 minutes

### Other Mediators
- 1-nitroso-2-naphthol-3,6-disulfonic acid
- 2,2,6,6-tetramethylpiperidine-1-oxyl (TEMPO)
- Various natural mediators (syringaldehyde, acetosyringone)

## Laccase-Catalyzed mycoremediation-of-pahs|PAH Degradation -- Comprehensive Data

### [[turkey-tail-trametes-versicolor]] Laccase (Majcherczyk et al., 1998)

| mycoremediation-of-pahs|PAH | Without Mediator | With HBT |
|-----|-----------------|----------|
| Anthracene | 37% (24h) | 91% (24h) |
| Benzo[a]pyrene | 3% (24h) | 100% (24h) |
| Acenaphthene | -- | 100% (24h) |
| Fluorene | -- | 100% (24h) |
| Perylene | -- | 100% (24h) |

### Coriolopsis gallica Laccase (Pickard et al., 1999)
- Anthracene: 100% degradation in 10 minutes with ABTS
- Demonstrates extremely rapid laccase-mediator systems

### Pycnoporus cinnabarinus Laccase (Rama et al., 1998)
- Benzo[a]pyrene: 95% degradation with ABTS mediator in 24h

### Pleurotus eryngii Laccase (Rodriguez et al., 2004)
- Benzo[a]pyrene: 70% degradation with HBT mediator
- Also produces hybrid MnP/LiP forms

### Trametes hirsuta Laccase (Majcherczyk and Johannes, 2004)
- Active in mycoremediation-of-pahs|PAH oxidation with mediator systems

## Metabolic Products from mycoremediation-of-pahs|PAH Degradation

Laccase-mediated mycoremediation-of-pahs|PAH oxidation produces identifiable metabolites:
- Anthracene -> anthraquinone
- Benzo[a]pyrene -> benzo[a]pyrene-3,6-dione, benzo[a]pyrene-1,6-dione, benzo[a]pyrene-6,12-dione
- Fluorene -> 9-fluorenone
- Phenanthrene -> phenanthrene-9,10-dione, 2,2'-diphenic acid
- Pyrene -> 1,6-pyrenedione, 1,8-pyrenedione

These metabolites are generally less toxic and more water-soluble than parent mycoremediation-of-pahs|PAHs, facilitating further biological degradation.

## Dye Decolorization by Laccase

### Dye Types Targeted
- Azo dyes (most common industrial dyes)
- Anthraquinone dyes
- Triphenylmethane dyes
- Indigo dyes
- Heterocyclic dyes

### Key Fungal Species for Dye Decolorization
- [[pleurotus-ostreatus]]: significant dye decolorization in solid-state fermentation
- Coriolopsis gallica: high laccase activity for dye treatment
- Bjerkandera adusta: effective for recalcitrant dyes
- Pycnoporus cinnabarinus: strong laccase production
- Ganoderma lucidum: dye decolorization capability

### Decolorization Mechanisms
1. **Enzymatic degradation**: laccase oxidizes dye chromophores, breaking conjugated systems
2. **Biosorption**: dye molecules adsorb to fungal biomass
3. **Synergistic action**: laccase + mediators + other enzymes (MnP, LiP)

### Factors Affecting Decolorization
- Dye structure (azo bonds are primary targets)
- pH (optimal varies by fungus, generally acidic 3-5)
- Temperature
- Presence of mediators
- Nitrogen source
- Carbon source and cosubstrates
- Dye concentration

## Wastewater Decolorization in Constructed Wetlands

Constructed wetlands employing fungal communities have been investigated for wastewater treatment:
- Surface constructed wetland cells receiving secondary-treated pulp mill effluent showed diverse microbial communities
- Decolorization depends on decomposer communities including fungi
- Passive, low-maintenance approach suitable for rural and remote applications
- Combination of fungal and plant systems enhances treatment efficiency

## Laccase Bioreactors

### Design Considerations
- Packed-bed reactors with immobilized laccase or fungal biomass
- Continuous-flow systems for industrial effluent treatment
- Temperature and pH control critical for enzyme stability
- Enzyme immobilization extends operational lifetime

### Industrial Applications
- Textile effluent treatment (azo dye removal)
- Pulp and paper industry (chlorophenol and colored compound removal)
- Olive mill wastewater treatment
- Pharmaceutical industry wastewater

### Enzyme Stability
- Laccase is generally more stable than LiP and MnP
- Can be immobilized on various supports (alginate, silica, polyurethane foam)
- Retains significant activity over multiple treatment cycles
- More amenable to heterologous expression than LiP

## Genetic Engineering of Laccase

Laccase genes have been cloned from multiple fungal species. Heterologous expression systems have been developed, making laccase more accessible for industrial applications than other [[turkey-tail-trametes-versicolor]] laccase genes well characterized
- Pycnoporus cinnabarinus laccase gene cloned
- Pleurotus spp. laccase genes identified
- Site-directed mutagenesis used to improve substrate range and stability
- Expression in yeast (Pichia pastoris) and other hosts demonstrated
