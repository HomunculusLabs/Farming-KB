...
**The laws explain but cannot predict**: These agricultural laws are of use only in explaining results, and cannot be of any help in reaching beyond what is currently possible. When the farmer asks specific questions about optimal planting density or timing, the laws can only offer post-hoc rationalizations about what went wrong.

**Cannot transcend current methods**: These laws cannot boost rice yields beyond those attainable by present methods. They are useful only in preventing reduced yields -- a defensive rather than generative function.
...
Comparable results have been documented by natural farming practitioners worldwide. In Thailand, farmers using Fukuoka's methods reported stable rice yields with zero external inputs after a 3-5 year transition period. In India, the Natural Farming movement led by Subhash Palekar has documented thousands of farms achieving yields comparable to or exceeding conventional methods using only locally available materials — cow dung, cow urine, legume intercropping, and mulch. These results consistently demonstrate that the barrel model's predicted relationship between inputs and outputs breaks down when the system's inherent biological fertility is allowed to function.
