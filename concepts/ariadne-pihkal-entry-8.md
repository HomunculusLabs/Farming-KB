---
title: ARIADNE (4C-DOM, Dimoxamine) - PIHKAL Entry #8
tags: [pihkal, amphetamine, dom-analogue, antidepressant, classic-ladies]
date: 2026-04-28
updated: 2026-04-28
sources:
  - /Users/t3rpz/wiki/raw/papers/shulgin-a-pihkal.md
---

# ARIADNE (4C-DOM, Dimoxamine)

**PIHKAL Entry #8**

## Chemical Identity

- **Full name:** 1-(2,5-Dimethoxy-4-methylphenyl)-2-aminobutane;
  2,5-Dimethoxy-alpha-ethyl-4-methylphenethylamine
- **Synonyms:** 4C-DOM, BL-3912, Dimoxamine
- **Class:** Amphetamine analogue (alpha-ethyl homologue of DOM)
- **Hydrochloride salt:** white crystals, mp 232.5-234.5 °C
- **Nitrostyrene intermediate:** brilliant orange crystals,
  mp 114-115 °C (after recrystallization from MeOH)

## Dosage and Duration

- **Dosage:** Unknown as psychedelic; 12-32 mg tested
- **Duration:** Short (5-6 hours to largely gone)

## Qualitative Effects

At 12 mg, a distinct mood improvement was noted with excellent
sleep that evening. The compound was described as physically
benign.

At 32 mg, a threshold effect lasting a couple of hours was
observed.

At 25 mg of the resolved "R" isomer, there was an alert of a
psychedelic without the full package, perhaps with some paranoia.
By the fifth hour, everything was largely gone.

ARIADNE is not really a stimulant, certainly not a psychedelic,
and yet there was something there. It was explored as a potential
psychotherapeutic ally; a friend found some value in patients with
underlying depression, but not much with others.

## Synthesis Notes

Dean-Stark condensation of 2,5-dimethoxy-4-methylbenzaldehyde
(from the 2C-D recipe) with 1-nitropropane and cyclohexylamine
in benzene (24 h reflux) yielded the nitrostyrene as brilliant
orange crystals (mp 114-115 °C, 19.6 g). Reduction with LAH
(12.5 g in 600 mL THF, 15 h reflux) gave the amine
hydrochloride (11.4 g).

The racemate was resolved into optical isomers:
- "S" isomer via (+)-2beta-nitrotartranilic acid salt
- "R" isomer via (+)-2beta-chlorotartranilic acid salt

The "R" isomer was also prepared independently by reductive
amination of 1-(2,5-dimethoxy-4-methylphenyl)-2-butanone
(obtained from the nitrostyrene and elemental iron) with
(+)-alpha-methylbenzylamine followed by hydrogenolysis of the
benzyl group.

## Safety and Pharmacology

ARIADNE is not hallucinogenic in cats (unlike DOM, which causes
dramatic tail erection). It showed re-motivation effects in
maze-running monkeys that had previously decided not to run
mazes anymore.

The "R" isomer was patented as an anti-depressant (tradename
Dimoxamine) for restoring motivation in senile geriatric
patients. Despite advocacy for the name "Ariadnamine," the
pharmaceutical tradename Dimoxamine was assigned. The compound
did not reach the commercial market.

Several dozen analogues were made by pharmaceutical companies
with two major variation strategies:
1. Keep the 4-methyl group; vary the alpha-position (propyl,
   butyl, dimethyl, phenyl, benzyl, phenethyl)
2. Keep the alpha-ethyl group; vary the 4-position (chloro,
   iodo, methylthio, carboxy)

The bromo-counterpart of ARIADNE
(1-(2,5-dimethoxy-4-bromophenyl)-2-aminobutane) was prepared
via bromination of the proteo compound in acetic acid, yielding
a hydrochloride salt with mp 204-206 °C. The isomeric
3,4-dimethoxy analogue was explored by the Chemical Warfare
group at Edgewood Arsenal (code EA-1322).

N,N-dialkylated derivatives were prepared as potential brain
blood-flow indicators for radio-iodine or radio-fluorine
labeling. These followed a common flow: benzaldehyde + nitropropane
(with N,N-dimethylethylenediamine catalyst) to nitrostyrene,
reduction with iron to ketone, then reductive amination with
dimethylamine hydrochloride/sodium cyanoborohydride.

## The Ten Classic Ladies

ARIADNE is the first of Shulgin's "Ten Classic Ladies," a
systematic program to replace each of the ten unique hydrogen
atoms in DOM with a methyl group, named alphabetically:

1. **ARIADNE** (#8): Methyl on the methyl group of the chain.
   Named for the Cretan princess who gave Theseus the thread to
   navigate the Labyrinth.
2. **BEATRICE** (#11): Methyl on the nitrogen. Named for Dante's
   Beatrice.
3. **CHARMIAN**: Methyl on the alpha-carbon (phentermine analogue).
   Named for Cleopatra's attendant (or a friend's wife). The MDA
   analogue is MDPH. Synthesized but very reduced potency.
4. **DAPHNE** and **ELVIRA**: Methyl on the beta-carbons (threo
   and erythro isomers). Started but not completed.
5. **FLORENCE**: 2-ethoxy (Italian for Firenze).
6. **GANESHA**: 3-methyl. An elephant-headed Indian God. Not
   actually feminine. Became a major starting point for potent
   compounds.
7. **HECATE**: 4-ethyl. Now known as DOET.
8. **IRIS**: 5-ethoxy. Goddess of the rainbow.
9. **JUNO**: 6-methyl. Synthesized but unexplored for 14 years.

## See Also

- [[dom-entity]]
- [[doet-entity]]
- [[beatrice-pihkal-entry-11]]
- [[bod-pihkal-entry-14]]
- [[do-family-compounds-pihkal]]
