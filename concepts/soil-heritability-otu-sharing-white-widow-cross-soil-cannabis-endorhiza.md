...
A central question in [[cultivar-specificity-plant-microbiomes|plant microbiome]] research is the degree to which root-associated microbial communities are inherited from the surrounding environment versus selected by host plant genetics. The [[two-tier-selection-model]] proposes a two-step process: first, bulk soil microbes serve as the available source pool, and second, host plant factors selectively filter and enrich specific taxa from that pool into the [[Rhizosphere|rhizosphere]] and [[cannabis-core-endorhiza-microbiome|endorhiza]].
