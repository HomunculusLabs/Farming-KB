---
title: "Jarrold Self-Humidifying Aquarium Chamber for Mushroom Fruiting"
slug: "jarrold-self-humidifying-aquarium-chamber"
tags: [mushroom-cultivation, humidity-chamber, diy, fruiting, jarrold, aquarium]
source: jarrold indoor mushroom growing technique boil a bag
created: 2026-05-10
---

# Jarrold Self-Humidifying Aquarium Chamber for Mushroom Fruiting

## Overview

D.H. Jarrold's boil-a-bag [[exotic-mushroom-cultivation-guide]] describes two [[humidity-chamber-designs-jarrold-mushroom-cultivation]] for the fruiting stage. The primary design uses a simple 4-foot by 2-foot fish tank as a [[passive-humidity-fruit-chamber-polyethylene-sheeting]] chamber, while a more advanced self-humidifying design incorporates an aquarium air pump and air stone for automated humidity maintenance. These DIY chambers represent accessible, low-cost approaches to maintaining the high-humidity environment required for mushroom fruiting.

## Passive Fish Tank Humidity Chamber

### Basic Setup

The simplest fruiting chamber in Jarrold's system is a standard glass aquarium:

- **Dimensions:** 4 ft × 2 ft (approximately 120 cm × 60 cm)
- **Construction:** Standard glass fish tank, no modifications required
- **Humidity source:** Layer of soaked hydro pellets (hydroton/LECA) on the bottom
- **Sealing:** Plastic sheet placed over the top to create a seal
- **Purpose:** Prevents humidity escape and excludes flies and contaminants

### Operation

The passive chamber requires minimal daily maintenance:

- Fan fresh air into the tank twice daily for gas exchange
- Monitor for signs of [[mushroom-pin-formation]]
- Ensure hydro pellets remain moist
- Watch for contamination indicators

### Bag Placement

Once mycelium has fully colonized the substrate bag, the filter can is removed and bags are placed into the chamber. Jarrold recommends cutting bags down the sides when mushrooms begin forming, leaving approximately 2 cm of the bottom intact. This allows the bags to stand upright in the chamber without the substrate becoming waterlogged from contact with the humid base.

## Self-Humidifying Chamber Design

### Components Required

The automated design adds active humidity control to the basic aquarium chamber:

| Component | Source | Purpose |
|-----------|--------|---------|
| Fish tank (4×2 ft) | Local hardware shop | Main chamber body |
| Air stone | Aquarium supply shop | Bubbles water for evaporation |
| Airline tubing | Aquarium supply shop | Connects pump to stone |
| Small aquarium air pump | Aquarium supply shop | Drives air through stone |
| 2mm plastic sheet | Hardware shop | Inner angled drip lids |
| Plastic car registration plate nuts and bolts | Car accessory shop | Mounting hardware |

### Assembly

The self-humidifying chamber is built by modifying the basic fish tank:

1. A small hole is drilled near the top of the tank for the airline tubing
2. The tubing is fed through to a water container at the bottom of the tank
3. An air stone is attached to the tubing end inside the water
4. The air pump is connected to the external end of the tubing
5. Inner angled drip lids are fabricated from 2mm plastic sheet

The angled drip lids serve to distribute condensation evenly back into the chamber rather than allowing it to pool on one surface. The car registration plate nuts and bolts are used as mounting hardware for the drip lids — an ingenious repurposing of readily available, cheap materials.

### How It Works

When the air pump runs, it pushes air through the airline tubing and out through the submerged air stone. The rising bubbles agitate the water surface, dramatically increasing the rate of evaporation. This moisture-laden air rises into the main chamber volume, maintaining high humidity without manual spraying or misting. The system is essentially a small-scale fog chamber powered by the same principle as aquarium aeration.

### Advantages Over Passive Design

- **Consistent humidity:** Continuous operation eliminates humidity fluctuations
- **Reduced maintenance:** No need to manually mist or top up water as frequently
- **Better gas exchange:** Air pump provides gentle air circulation
- **Lower contamination risk:** Sealed system with filtered air input reduces [[mushroom-contamination-vectors]]
- **Scalability:** Air pump capacity can be matched to chamber size

## Warm Humid Air Delivery Chamber

For cultivators in cold climates or those growing during cold seasons, Jarrold describes a third, more advanced design:

### Design Features

- Water-filled base container with heating element
- Input fan connected to the main chamber via large duct
- Multiple small connection ducts as potential outlets
- Timer-controlled water heater to regulate [[greg-green-temperature-and-humidity-control]]
- Water heater timer requires several hours of initial calibration
