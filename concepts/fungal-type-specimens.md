...
the anamorph state, and connecting these to their teleomorphs has been a major
activity in fungal systematics. The 2011 International Botanical Congress in
Melbourne established one name for one fungus, ending the dual nomenclature
system that previously permitted separate names for teleomorph and anamorph
...
- **Poorly preserved specimens**: Historical specimens may be fragmented,
  contaminated, or deteriorated beyond useful examination. Early collectors
  sometimes failed to document collection details adequately, reducing the
  ecological and geographic context of type material.

- **Ambiguous original descriptions**: Many species were described from
  brief Latin diagnoses or minimal morphological characterizations that do
  not adequately distinguish the taxon from similar species. Without clear
  diagnostic characters, interpreting the intended concept of a species
  from its type can be difficult or impossible.

- **Mixed collections**: Type material sometimes contains more than one
  fungal species, particularly when collected from complex natural
  substrates. Lectotypification allows selection of a single element from
  such mixed collections, but this requires careful examination and
  justification.

## Molecular Data from Type Specimens
The extraction and sequencing of DNA from historical type specimens has
become a transformative tool in fungal taxonomy:

- **[[fungal-dna-extraction-methods]] from herbarium specimens**: Modern extraction protocols
  can recover amplifiable DNA from dried fungal specimens that are decades
  to centuries old. However, DNA from older specimens is often fragmented
  and chemically modified, requiring specialized PCR approaches including
  mini-barcoding, primer walking, and whole genome amplification.

- **Type-derived reference sequences**: Sequences obtained from verified
  type specimens provide the most authoritative reference for species
  identification. The UNITE database for fungal ITS sequences gives highest
  priority to sequences derived from ex-type cultures or verified type
  specimens, designating these as species hypotheses with the highest
  confidence level.

- **Epitypification with molecular data**: When existing type material
  is ambiguous or degraded, researchers may designate an epitype supported
  by molecular data. The epitype is a freshly collected specimen from the
  same geographic region and host as the original type, with accompanying
  DNA sequence data that fixes the species concept unambiguously.

- **Recalcitrant specimen types**: Some historical specimens resist DNA
  extraction due to chemical fixation methods, chemical degradation, or
  extensive contamination. In these cases, sequence data from ex-type
  living cultures or recently collected topotypic material provides the
  best available molecular reference for the species name.

## The Role of Herbaria in Fungal Taxonomy
Herbaria serve as the permanent repositories for fungal type specimens and
are fundamental infrastructure for taxonomic research:
