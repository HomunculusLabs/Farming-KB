---
title: Flush-Dependent Alkaloid Variation in Psilocybe
slug: flush-dependent-alkaloid-variation-psilocybe
source: variation-of-psilocybin-and-psilocin-levels-bigwood-beug
tags:
  - psilocybin
  - psilocin
  - alkaloid
  - flush-variation
  - hplc
  - psilocybe-cubensis
  - cultivation
  - potency
created: 2026-05-10
---

# Flush-Dependent Alkaloid Variation in Psilocybe

## Overview

Bigwood and Beug (1982) demonstrated that [[hplc-analysis-of-psilocybin-and-psilocin]] levels
in *[[basidiocarp-four-stage-development-classification-psilocybe-cubensis-badham-1982]]* vary substantially across [[psilocin-biosynthetic-delay-hypothesis-successive-flushes]].

Their controlled miniculture study revealed that flush number is a critical variable in determining alkaloid content, particularly for psilocin, which was often absent in early harvests.

These findings have direct implications for dosage prediction and for understanding variations in subjective effects among users.

## Source Study

**Citation:** Bigwood, J. & Beug, M. W. (1982). Variation of psilocybin and psilocin levels with repeated flushes of *[[basidiocarp-four-stages-psilocybe-cubensis-morphological-development]]* (Earle) Singer. *Journal of Ethnopharmacology*, 5(3), 287–291.

The study examined alkaloid variation across repeated flushes using controlled miniculture techniques and HPLC analysis calibrated against NIDA reference standards.

This methodological rigor distinguishes the study from many informal reports of [[psilocybe-cubensis-potency-variation-by-flush]] in the mycological literature.

## Cultivation Method

The miniculture protocol used a simple, reproducible grain-based system designed for controlled experimentation.

Each culture vessel contained 10 g of rye grain hydrated with 15 ml of water, prepared in half-pint mason jars.

Jars were autoclaved for sterility, then inoculated from agar cultures to ensure genetic consistency of the [[shirota-4-hydroxyindole-acetyl-protection-psilocin-synthesis-starting-material]].

During colonization, jars were shaken every 4 days over a 28-day period to promote even mycelial distribution.

After full colonization, the grain was cased with a mixture of 2 parts peat, 1 part calcium carbonate (CaCO₃), and 2 parts perlite or vermiculite.

The first flush typically appeared 4–5 weeks after inoculation, with subsequent flushes following at regular intervals.

## Analytical Approach

Harvested mushrooms were freeze-dried and stored at −5°C prior to analysis to preserve alkaloid integrity over the duration of the study.

Quantification was performed using high-performance liquid chromatography (HPLC), calibrated directly against NIDA reference standards.

Freeze-drying [[root-cellaring-guide]] prevented oxidative degradation of psilocin, which is notably less stable than psilocybin at room temperature.

## Flush-Dependent Psilocin Pattern

The most striking finding was the strong flush dependence of psilocin levels across the harvest cycle.

Psilocin was generally absent from the first flush and sometimes absent from the second flush as well.

Psilocin content increased progressively with subsequent flushes, reaching maximum concentration by the fourth flush.

After the fourth flush, psilocin levels began to decline, suggesting a bounded biosynthetic window tied to the physiological state of the culture.

This pattern indicates that psilocin biosynthesis is regulated differently from psilocybin and may be linked to substrate maturation or metabolic shifts in the aging fungal culture.

The practical consequence is that early-flush harvests produce notably different psychoactive profiles than later ones, even when total alkaloid content appears similar on a dry-weight basis.

## Psilocybin Variability

Unlike psilocin, psilocybin showed no consistent upward or downward trend across successive flushes.

Instead, psilocybin levels varied unpredictably by a factor of approximately four across all samples tested.

Measured psilocybin concentrations ranged from 3.2 to 13.3 mg/g dry weight [[psilocin-accumulation-pattern-across-flushes]], a remarkably wide band for a single strain under [[psilocybe-cubensis-strain-potency-variability-controlled-conditions-bigwood-beug]].

Psilocybin was nearly always present at least twice the concentration of psilocin in any given sample.

This high degree of variability means that the same strain grown under nominally identical conditions can produce substantially different potencies from one flush to the next.

## Cap vs. Stem Distribution

The study also examined how alkaloids were distributed between caps and stems across different strains of *P. cubensis*.

In the M.R. and Ecuadorian strains, caps contained approximately twice the psilocybin concentration found in stems.

In these same two strains, small amounts of psilocin were detected exclusively in the stems, with none found in the cap tissue.
