...
Source: [[jadam-local-material-collection-and-use]] difficult to
   ...
   can exceed reserves if used for procreative assets only (e.g. forests).
   Forests represent firewood, but reserves are never cut except in dire
   emergencies.
3. Educate local businesses and banks (exchanges) in the use of the currency,
   which can be changed for national currency at the bank, or for other
   regional currencies. Post exchange rates publicly.
4. Start satisfying demand (e.g. for homes) by lending the currency to builders,
   who can exchange it at the bank for other currency needed in trade, or
   locally for products and services.
5. Producers of local goods accept local currency only; this creates demand,
   and other regions must "buy" such currency to obtain local goods.
6. To prevent hoarding, notes can be dated and a new issue made every 4 to 5
   years; this is also a check on unwarranted accumulation of wealth.
## Community Banking Principles
Mollison's model for community banking draws on historical precedents including
the Wörgl experiment in Austria (1932-33), where a stamped currency stimulated
local trade and reduced unemployment during the Great Depression. Community
banks serve as exchange institutions that hold reserves, issue currency, and
facilitate local trade. Unlike conventional banks, community banks are owned
and governed by their members, with profits returned to the community rather
than extracted by external shareholders. They operate on the principle that
money should circulate within the local economy rather than being siphoned to
distant financial centres.
