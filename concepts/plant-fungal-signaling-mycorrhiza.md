---
title: Plant-Fungal Signaling in Mycorrhizal Symbiosis
created: 2026-04-16
updated: 2026-04-16
type: concept
tags: [reference]
sources:
  - "raw/papers/permaculture-a-designers-manual-bill-mollison.md"
---
# Plant-Fungal Signaling in [[mycorrhizal-fungi]] that precedes and maintains the symbiotic relationship. This signaling involves plant-derived compounds that activate fungal metabolism and fungal signals that modulate plant development and immunity. The communication shares surprising molecular parallels with both plant-pathogen interactions and rhizobial symbiosis.

## Presymbiotic Signaling

Before physical contact between plant and fungus, a chemical dialogue establishes compatibility and prepares both organisms for colonization.

### Plant-to-Fungus Signals

#### Strigolactones

Strigolactones are the primary plant signals that activate AMF:

- Root-exuded sesquiterpene lactones that stimulate AMF spore germination and hyphal branching at extremely low concentrations (10^-13 to 10^-10 M)
- Originally identified as germination stimulants for root parasitic plants (Striga, Orobanche)
- Synthesized from carotenoid precursors; their production increases under phosphorus deficiency
- Also function as endogenous plant hormones regulating shoot branching and root architecture
- The dual role as both signals to fungi and internal hormones links symbiosis to plant nutritional status

#### Flavonoids

Flavonoid compounds serve multiple signaling roles:

- Stimulate hyphal growth of AMF (e.g., in Gigaspora margarita, flavonoids promote growth under CO2-enriched conditions)
- Are present in [[beta-glucans]]) can trigger defense responses
- The plant response to these elicitors is typically weak and transient compared to pathogen-derived elicitors
- Chitinase activity increases in roots during both AMF and EMF colonization
- This "defense paradox" — activation of defense-like responses during mutualistic symbiosis — is a recurring theme

## Symbiotic Signaling (During Colonization)

### Hormonal Signaling

#### Auxin

- AMF colonization modifies root auxin levels and sensitivity
- Some mycorrhizal fungi produce IAA directly (e.g., [[piriformospora-indica]], Tuber spp.)
- Auxin signaling may facilitate arbuscule development and root architectural changes
- MAP kinase cascades respond to auxin during early colonization events

#### Ethylene

- Ethylene is generally considered a negative regulator of mycorrhizal colonization
- High ethylene levels can suppress AMF colonization
- Some PGPR promote mycorrhization partly by reducing ethylene levels through ACC deaminase activity
- The ethylene pathway intersects with jasmonic acid signaling in modulating the symbiosis

#### Jasmonic Acid and Salicylic Acid

- JA/SA signaling pathways are involved in the balance between defense and mutualism
- JA signaling may be required for proper arbuscule development
- SA-mediated SAR generally antagonizes mycorrhizal colonization
- The crosstalk between JA and SA pathways determines the outcome of the plant-fungus interaction

### Calcium Signaling

- Calcium spiking in the nucleus is the earliest detectable plant response to fungal signals
- The calcium/calmodulin-dependent protein kinase CCaMK/DMI3 decodes calcium oscillations
- Calcium signaling coordinates the expression of symbiosis-specific genes
- Calcium influx also occurs in response to pathogen elicitors, creating specificity challenges

## Defense Regulation in Symbiosis

### The Mutualism-Parasitism Continuum

Molecular events in mycorrhizal signaling overlap with plant-pathogen interactions:

- Signal perception, signal transduction, and defense gene activation occur in both
- In mycorrhizal symbiosis, these responses are characteristically weak and transient
- The low intensity may reflect limited capacity of AMF to trigger defense
- Alternatively, the fungus may actively suppress defense responses through effector proteins

### Transient Defense Response

The defense response during mycorrhizal colonization shows characteristic temporal dynamics:

- Early phase: Defense gene activation (peroxidases, chitinases, PR proteins, GSTs)
- Mid phase: Suppression of defense as compatibility is established
- Late phase: Re-emergence of some defense markers during arbuscule senescence

### Glutathione-S-Transferases (GSTs)

GSTs exemplify the complex defense regulation in AM symbiosis:

- Multiple GST isoforms show differential regulation during colonization
- Some GSTs accumulate at early stages, before visible fungal colonization
- Tau-class GSTs are downregulated in response to both G. mosseae and G. intraradices
- GSTs may participate in arbuscule development, defense, or degradation processes

## Genes and Proteins in Signaling

### Transcriptomic Evidence

SSH (suppression subtractive hybridization) libraries from Medicago truncatula/G. mosseae interactions identified genes involved in:

- Signal transduction (MAP kinases, GTP-binding proteins, serine/threonine kinases)
- Defense response (peroxidases, GSTs, chitinases)
- Primary metabolism (alanine aminotransferase, NAD(P)H-dependent enzymes)
- Secondary metabolism (chalcone reductase, phenylpropanoid pathway enzymes)

### Proteomic Evidence

2-DE and MALDI-TOF proteomics confirmed several signaling proteins:

- GTP-binding proteins involved in signal transduction and growth regulation
- Peroxidases and peroxidase precursors as defense markers
- Serine/threonine kinases for environmental adaptation
- Ribosomal proteins indicating translational activation

## Signaling During Arbuscule Life Cycle

Arbuscules are transient structures with a defined life cycle:

1. **Initiation**: Fungal hypha contacts root cortical cell; signaling triggers arbuscule branching
2. **Development**: Arbuscule branches fill the cell; periarbuscular membrane forms around branches
3. **Maturity**: Active nutrient exchange phase; plant and fungal transporters are expressed
4. **Senescence**: Arbuscule degenerates; the host cell may recover or undergo programmed cell death

Each stage involves distinct signaling events, and the temporal regulation of these signals determines the outcome of each individual arbuscule.
