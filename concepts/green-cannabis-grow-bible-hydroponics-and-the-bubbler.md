---
title: Hydroponics Systems and the Bubbler - Greg Green's Cannabis Grow Bible
created: 2026-04-26
tags:
  - hydroponics
  - dwc
  - bubbler
  - [[deep-water-culture]]
  - growing-mediums
date: 2026-04-25
updated: 2026-04-25
sources:
  - raw/papers/greg-green-the-cannabis-grow-bible.md
type: concept
---

# Hydroponics Systems and the Bubbler
Chapter 9 of [[greg-green]]'s Cannabis Grow Bible provides a comprehensive
introduction to [[hydroponic-cannabis]] cultivation, covering the basic
principles, common system types, nutrient management, and detailed
instructions for building a simple but effective "bubbler" system.

## What Is Hydroponics

Hydroponics is the technique of growing plants without soil, using beds
of sand, gravel, or artificial mediums flooded with a nutrient solution.
The word comes from Greek: "hydro" (water) and "ponos" (labor).
A basic hydroponics system consists of:
- A pot or growing container
- A nutrient reservoir
- A grow medium
- A water pump
- Growth nutrients

The system has a top layer (grow medium) and a lower layer (water with
nutrients). Roots grow through the medium into the nutrient solution.
Water and nutrients are pumped in at timed intervals. Plants drink the
solution down, exposing roots to air. This allows plants to devote more
energy to upper growth rather than root searching.

## Advantages of Hydroponics

- Can produce a flowering plant in 3/4 the time of a soil grow.
- Produces massive root clusters (a 2-gallon bucket of roots from a
  single plant is common).
- Bigger and better buds compared to soil grows of the same strain.
- Over 100 different DIY systems exist, with about 15 proven useful
  for cannabis.
The biggest risk: if a hydroponics system is not well maintained, the
whole unit may fail and kill plants very quickly.

## Hydroponics Growing Mediums

### Rockwool

The most popular hydroponic medium, available in slabs or cubes (1 to 6
inches). Slabs can be cut to fit any pot or container. Rockwool should
be pre-soaked for 24 hours in water with a pH of 5.6 before use.

### Other Mediums

Growers also use clay pebbles, coconut coir, perlite, and various
artificial mediums designed to hold minerals, provide drainage, and
allow air to reach the roots.

## Hydroponics Systems

### NFT ([[nutrient-film-technique]])

An all-in-one system that is generally flat and long. A constant flow of
nutrients runs to the roots and back to the reservoir. The reservoir,
pumps, and nutrient solution are all contained within the same system.

### Flood and Drain ([[ebb-and-flow]])

Recognized by its depth. The grow medium sits above the reservoir, which
pumps nutrients to the roots at set intervals. During the day, plants
go through spells of dryness as the solution drains back to the
reservoir. The whole unit recycles nutrient solution at timed intervals.

### Drip Irrigation System

Feeds plants individually through small drippers. Plants are in separate
chambers, and the solution drains through much like an Ebb and Flow
system.

### Aeroponics

Roots are suspended in air and misted with nutrient solution. This
method provides maximum oxygen to roots but requires precise timing and
maintenance.

### Automatic and Manual Hydroponic Pots

Various commercial pots that automate or simplify the hydroponic process
for home growers.

## Hydroponics Nutrients

The most popular nutrient line for cannabis cultivation is the G.H.
Flora Series, a 3-part system (Gro, Micro, and Bloom). Experienced
growers adjust these to get optimum performance.

### Grow vs Bloom Formulations

- Grow solution: Used during vegetative stage.
- Bloom solution: Used during flowering stage. Contains more Phosphorus
  and Potassium, less Nitrogen.
- Some packs are complete all-in-one formulas.

### Nutrient Strength

Cannabis finds full-strength nutrient solutions too strong. Even medium
strength can burn plants:
- 3.5ml of A, B, and C per liter = 100% strength.
- Same amount in 2 liters of water = 50% strength.
- Cannabis grows very well between 30% and 50%.
- Never go full strength on your first grow.
- Start at 30% and increase if needed.

The most common hydroponics problem is plant burns from overfeeding.
Underfeeding is rarely reported.

## Hydroponics pH

Cannabis in hydroponics prefers a pH of 5.2 to 6.3 (compared to 7.0
for soil). pH can slip quickly in hydroponics systems. Use commercial
pH Up and pH Down adjusting solutions. It is easier to maintain a pH
range than a set level. Check pH as often as possible.

## Monitoring Recycle Systems

As plants extract nutrients, the reservoir becomes depleted. Use a PPM
(parts per million) reader or TDS meter to monitor nutrient levels.
Without a meter, replace the reservoir more frequently.

### Topping Up

If starting with 10 gallons, top up to 10 gallons every few days. Track
your initial mix ratio and add proportionally when topping up. Keep a
mixing chart for different quantities.

## Algae Prevention

Keep reservoirs away from direct light exposure. Most commercial
systems are manufactured "light tight." For DIY systems, use thick
black PVC tape or garbage bags to cover the lid and reservoir. If algae
develops, clean the system and replace the nutrient solution.
