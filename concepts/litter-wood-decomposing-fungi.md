---
created: 2026-04-12
source: biodiversity-of-fungi (Hyde et al. Chs. 9-10, Boddy/Frankland/Watkinson)
sources: []
tags:
- decomposition
- wood-rot
- leaf-litter
- saprotrophs
- carbon-cycling
- forest-ecology
title: Litter and Wood Decomposing Fungi
type: concept
updated: 2026-04-12
---

# Litter and Wood Decomposing Fungi

Fungi are the primary decomposers of plant litter and wood in terrestrial ecosystems, driving the global carbon cycle through enzymatic breakdown of cellulose, hemicellulose, and lignin. The fungal communities on decomposing plant material follow predictable successional patterns shaped by resource chemistry and competitive interactions.

## Plant Litter Decomposition

### Litter Quality and Decomposition
- **C:N ratio**: High C:N litter (e.g., pine needles, >60:1) decomposes slowly; low C:N litter (e.g., legume leaves, <20:1) decomposes rapidly
- **Lignin content**: High lignin limits decomposition rate; lignin must be degraded before cellulose becomes accessible
- **Polyphenols**: Tannins and other polyphenols inhibit decomposition by binding proteins and enzymes
- **Physical structure**: Thick cuticles and tough leaves resist fungal penetration

### Succession on Leaf Litter

**Stage 1 — Colonization (days to weeks)**:
- Sugar fungi (Mucorales, yeasts) exploit readily available soluble sugars
- Rapid colonization by fast-growing, opportunistic species
- *Cladosporium*, *Epicoccum*, *Aureobasidium* on leaf surfaces

**Stage 2 — Primary Decomposition (weeks to months)**:
- Cellulolytic and hemicellulolytic fungi dominate
- Soft-rot fungi (Ascomycetes and Deuteromycetes) degrade cellulose and hemicellulose
- Key genera: *Trichoderma*, *Penicillium*, *Fusarium*, *Cladosporium*
- White-rot basidiomycetes may begin appearing

**Stage 3 — Secondary Decomposition (months to years)**:
- Lignin-degrading fungi (white-rot Basidiomycetes) become important
- Slow-growing, competitive, ligninolytic species
- Key genera: *Phanerochaete*, *Bjerkandera*, *Trametes*, *Stereum*
- Fungi that can tolerate low-quality, lignin-rich substrates

**Stage 4 — Humus Formation (years)**:
- Resistant material (humus) decomposes very slowly
- Specialized fungi with oxidative enzyme systems
- [[mycorrhizal-fungi]] may contribute to humus decomposition (see [[mycorrhizal-types]])

### Factors Affecting Litter Decomposition

**Environmental**:
- Temperature: Q10 of ~2 for decomposition rate (rate doubles per 10°C increase)
- Moisture: Optimal near field capacity; both drought and waterlogging inhibit
- [[soil-ph]]: Acidic soils slow decomposition; neutral to slightly acidic optimal
- Oxygen: Anaerobic conditions slow decomposition and shift to bacterial/fermentative processes

**Chemical**:
- Lignin:N ratio is often the best predictor of decomposition rate
- Polyphenol content negatively correlated with decomposition
- Nutrient availability (especially N and P) affects fungal activity

**Biological**:
- Fungal species identity matters; different fungi have different enzyme capabilities
- Invertebrate activity (mites, springtails, millipedes) fragments litter, increases surface area
- Faecal pellets from litter-feeding invertebrates are hotspots of fungal activity

## Wood Decomposition

### Wood as a Substrate
- Wood is composed of: cellulose (~40-50%), hemicellulose (~20-30%), lignin (~20-30%)
- Variable composition by species: softwoods (gymnosperms) have more lignin, less hemicellulose; hardwoods (angiosperms) have more hemicellulose
- Wood is nutrient-poor (high C:N ratio, often >400:1), which limits microbial activity
- Moisture content determines fungal activity: ~30-60% moisture content optimal

### Types of Wood Decay

**White Rot** (see [[fungal-decomposition]]):
- Degradation of both cellulose and lignin
- Caused by Basidiomycetes (primarily)
- Wood becomes lighter in color, soft, spongy
- Enzymes: lignin peroxidase (LiP), manganese peroxidase (MnP), laccase
- Key genera: *Phanerochaete*, *Trametes*, *Bjerkandera*, *Irpex*, *Pleurotus*, *Piptoporus*
- Complete decomposition of all wood components is possible

**Brown Rot**:
- Selective removal of cellulose and hemicellulose; lignin modified but not removed
- Caused by Basidiomycetes (primarily)
- Wood becomes brown, cracked, cubical, brittle
- Mechanism: Fenton chemistry (H2O2 + Fe2+ -> OH radical) generates hydroxyl radicals that cleave cellulose
- Key genera: *Serpula*, *Coniophora*, *Gloeophyllum*, *Wet rot* fungi: *Poria*, *Postia*, *Antrodia*
- Primarily on softwoods (conifers)
- Does not degrade lignin; leaves brown, lignin-rich residue

**Soft Rot**:
- Caused by Ascomycetes and Deuteromycetes
- Cavities formed within secondary cell wall (Type I) or erosion from lumen surface (Type II)
- Important in wet wood, preservative-treated wood, and aquatic environments
- Key genera: *Chaetomium*, *Trichoderma*, *Xylaria* (Ascomycetes)
- More important in tropical than temperate regions
- Can degrade wood that is too wet or too dry for basidiomycetes

### Succession on Wood

**Fresh wood (0-1 year)**:
- Endophytic fungi already present in living wood begin to act as pioneer decomposers
- Airborne spores colonize exposed surfaces
- Fast-growing, broad-spectrum decomposers: *Trichoderma*, *Mucor*, *Penicillium*
- Blue-stain fungi (Ophiostomatales) colonize via bark beetle vectors

**Early decomposition (1-5 years)**:
- Primary decay fungi establish: white-rot and brown-rot basidiomycetes
- Competition between species determines community composition
- *Stereum*, *Trametes*, *Bjerkandera* (white rot) and *Gloeophyllum*, *Serpula* (brown rot) common

**Late decomposition (5-50+ years)**:
- Dominant decay fungus monopolizes resource
- Mycelial cords and rhizomorphs extend foraging range
- *Armillaria* species: aggressive, long-lived; can form some of Earth's largest organisms (see [[fairy-rings-and-giant-mycelia]])
- Resource becomes increasingly depleted; succession slows

**Very late stage**:
- Woody residue approaches humus
- Secondary colonizers on residual material
- Mycorrhizal fungi may utilize remaining organic matter

## Competition and Combat Between Fungi

Fungi compete intensely for wood resources. Interactions include:

### Interactions at Mycelial Fronts
- **Intermingling**: Hyphae grow through each other without apparent effect
- **Deadlock**: Hyphae meet and stop growing; stable interface forms
- **Replacement**: One species overgrows and replaces another
- **Bystander effect**: Fungi release volatile organic compounds that inhibit distant competitors

### Combat Mechanisms
- **Antibiotic production**: Trichoderma produces trichodermin, gliotoxin
- **Enzyme secretion**: Laccases, proteases, chitinases attack competitors
- **Physical contact**: Hyphal coiling, penetration, lysis
- **Resource sequestration**: Rapid colonization of new territory
- **Secondary metabolites**: Many basidiomycetes produce antifungal compounds

### Outcomes
- Combat outcomes depend on: relative growth rates, resource status, moisture, temperature
- Replacements are more common when one fungus has occupied the resource longer
- Community composition in wood is largely determined by competitive hierarchy
