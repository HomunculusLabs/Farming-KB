...
All materials are measured out by volume, that is, by the scoop, bucketful, jar full, or similar container. Proportions varying plus or minus 10 percent from the ideal ratios will still work out to be an effective fertilizer. This volume-based approach makes the formula forgiving and accessible to gardeners without precise scales.

The standard COF recipe consists of:
- 4 parts seed meal (any kind except copra meal, which is one-third weaker)
- 1 part agricultural lime
- 1/2 part gypsum (or double the agricultural lime if gypsum is omitted)
- 1/2 part dolomite lime
- 1 part phosphorus source (rock phosphate, bone meal, or high-phosphate guano)
- 1 part kelp meal (or basalt dust as substitute)

A higher-nitrogen variant replaces 3 parts of seed meal with 3 parts tankage (blood-and-bone, meat meal) for spring leafy crops.
...
## Tankage or Blood and Bone (Higher-Nitrogen Option)

For gardeners wanting a higher nitrogen mix, particularly for leafy crops in spring, tankage (also called blood-and-bone, meat meal, or meatmeal) can partially replace seed meal. Tankage is a byproduct of slaughterhouses, consisting of the whole animal minus the fat, hide, and bones. It typically analyzes around 10-4-0.

Solomon notes that tankage quality varies. Some lots may be higher in nitrates, requiring slightly larger quantities to achieve the same effect. Despite this variability, tankage is a potent nitrogen source that complements the slower-releasing seed meals. The higher nitrogen option is described as "slightly better for leafy crops in spring" and provides a faster nitrogen boost when cool soil temperatures slow the decomposition of seed meal.

The tankage variant uses:
- 3 parts seed meal
- 3 parts tankage (with 4 parts copra meal supplemented by 1 part tankage as an alternative)
...
There are three types of lime relevant to gardening. Agricultural lime is relatively pure calcium carbonate. Dolomitic lime contains both calcium and magnesium. Gypsum is calcium sulfate. Solomon warns against using quicklime, burnt lime, hydrated lime, or other chemically processed limes, as these can be caustic and damaging to soil biology.
...
## Slow Release and Soil Biology

Complete decomposition of COF in warm soil takes approximately two months. During that period, nutrients are steadily being released through the activity of the complex ecology of living creatures in the soil. The rate of decomposition is determined by soil temperature, roughly doubling for each 10-degree Fahrenheit increase. Gardeners in hot climates get faster decomposition and bigger results from smaller applications, though the effect lasts for a shorter time.

This slow-release characteristic is fundamentally different from chemical fertilizers, which dissolve rapidly in water and provide a brief burst of nutrient availability followed by a rapid decline. The biological decomposition pathway means that nutrients become available in sync with plant demand, reducing leaching losses and maximizing uptake efficiency.

## Key Considerations

- COF is measured by volume (scoops, bucketfuls) not by weight, making proportions forgiving
- Routine use of COF eliminates the need for separate pH correction liming
- The formula automatically distributes approximately 25 kilograms of lime materials per 100 square meters per year
- Complete decomposition takes about two months in warm soil, providing sustained release
- Hot-climate gardeners get faster results from smaller applications with shorter duration
- Buying all ingredients in bulk sacks is essential for economic viability
