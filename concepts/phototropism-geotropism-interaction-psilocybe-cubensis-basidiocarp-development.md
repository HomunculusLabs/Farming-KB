---
title: Phototropism-Geotropism Interaction in P. cubensis Development
created: 2026-05-10
tags: [mycology, tropism, phototropism, geotropism, psilocybe-cubensis, basidiocarp, badham-1982]
---

# Phototropism-Geotropism Interaction in P. cubensis Development

## Overview

The interplay between phototropism and geotropism [[cap-and-stem-alkaloid-distribution-in-psilocybe-cubensis-strains]]
represents one of the most complex tropic systems in basidiomycete fungi.
Badham's 1982 wind tunnel experiments demonstrated two distinct tropic
phases during [[four-stage-basidiocarp-development-psilocybe-cubensis-badham]], with a critical transition linked
to spore production. This interaction determines final fruiting body
orientation and directly affects spore dispersal efficiency.

## Two-Phase Tropic Model

**Phase 1 — Anemotropism (Stage 1 to Stage 2):**
During early development, the stipe elongates toward the direction of
greatest evaporative demand. In wind tunnel conditions, growth is directed
into the airflow regardless of light direction, fertility, or initial stipe
orientation. The mechanism likely involves differential cell expansion
driven by moisture gradients across the stipe surface.

**Phase 2 — [[coprophilous-habitat-negative-geotropism-spore-dispersal-adaptation]] (Stage 2 to maturity):**
At Stage 2 (nearly spherical pileus), a second tropism emerges that masks
Phase 1. The pileus recurves upward from the wind direction with lamellae
facing downward. This response is closely linked to spore production and
does not occur in sterile mutants or in complete darkness.

## Light's Role in Tropic Transitions

Light plays multiple roles in P. cubensis tropic behavior:

- **[[light-basidiocarp-initiation-psilocybe-cubensis-research]]:** Required for primordia formation (Badham,
  1980). Dark-grown cultures do not form fruiting bodies.

- **Spore production:** Necessary for spore formation. Darkness-grown
  cultures produced few spores and no Phase 2 [[spore-formation-geotropic-response-psilocybe-cubensis-badham-1982]].

- **Geotropic sensitivity:** Light may be required only indirectly,
  through its role in enabling spore production and gill maturation.

- **Stipe elongation:** Light inhibits stipe elongation somewhat, though
  experimental intensity (0.002 cal/cm2/min) was below field conditions.

- **Evaporative demand:** Light heats the mushroom surface, increasing
  evaporation. Under strong light, this could override wind as the
  dominant factor in stipe tropic curvature.

## Gill Hormone Hypothesis

Multiple evidence lines support a gill-derived hormone modulating tropisms:

- **Decapitation experiments** (Gruen 1963, Hagimoto 1963) linked tropic
  hormones to lamellae in Agaricus bisporus.

- **Agar diffusates** from gills partially replace pileus influence
  (Hagimoto and Konishi 1960), indicating a diffusible substance.

- **Sporeless mutants** in Badham's study lacked Phase 2 geotropism,
  directly connecting spore production (and functional lamellae) to
  the tropic transition.

- **Higher plant hormones** (auxins, gibberellins) occur in some mushrooms
  but do not regulate tropisms (Konishi and Hagimoto 1961; Pegg 1973).

## Developmental Stages

Badham defined four developmental stages with distinct tropic behaviors:

1. **Stage 1:** Slight pileus swelling. Strong anemotropic response into
   the wind. Occurs in light or darkness, fertile or sterile.

2. **Stage 2:** Pileus becomes spherical. Phase 2 negative geotropism
   emerges — pileus recurves upward, lamellae face down. Requires light
   and fertile (spore-producing) basidiocarps.

3. **Stage 3:** Pileus opens hemispherically. Spore formation begins.
   Geotropic response fully expressed.

4. **Stage 4:** Pileus becomes plane. Full maturity with active dispersal.

## Rotation Experiments

Key experiments revealing wind vs. gravity contributions:

- **Perpendicular rotation** (3 rpm, stipe perpendicular to wind): Growth
  directed upward — gravity dominates when wind is equalized across all
  sides of the stipe.

- **Parallel rotation** (3 rpm, stipe parallel to wind): Stipe elongation
  with pileus opening into wind. Horizontal growth under constant light
  with equalized gravity.

- **No rotation:** Strong anemotropic bending into wind (Phase 1), then
  Phase 2 recurvature at Stage 2.

## Phycomyces Parallels

P. cubensis tropisms parallel Phycomyces blakesleeanus responses:

- Both exhibit anemotropism plus avoidance responses (Cohen et al. 1975).
- Wind and water vapor are critical to Phycomyces tropisms (Johnson and
  Gamow 1971), analogous to the [[evaporative-demand-mechanism-fungi]] proposed
  for P. cubensis.
- Ethylene mediates Phycomyces avoidance (Russo et al. 1977) — volatile
  signaling may also operate in P. cubensis.
- Phycomyces responds to gravity and centrifugal force (Dennison 1961).

## Adaptive Significance

- **Spore dispersal:** Anemotropism positions the pileus in airflow for
  optimal dispersal; geotropic recurvation orients lamellae downward.
- **Cespitose spacing:** Growth toward evaporative demand increases
  spacing between clustered stipes, reducing competition.
- **Habitat instability:** Negative geotropism ensures horizontal pileus
  on unstable coprophagous substrates (Plunkett 1961).

## Related Topics

- basidiocarp morphogenesis mushroom fruiting body development
- mushroom phototropism light directed growth fungi
- negative geotropism mushroom stipe elongation gravity response
- mushroom hormone gill derived tropic regulation
- spore production tropic transition psilocybe cubensis
