...
Pure strains should be saved as "stock cultures" (slants) in test tubes stored at 35-40°F (2-4°C). Vitality should be checked every six months. The buddy system — passing duplicates to a cultivator friend — provides insurance against total strain loss.
...
Beyond slant storage and mineral oil preservation, modern cultivators have additional options for long-term strain preservation. Lyophilization (freeze-drying) can preserve fungal cultures for decades, though it requires specialized equipment and is more commonly used for commercial culture collections. Cryopreservation in liquid nitrogen at -196°C is considered the gold standard for long-term preservation, maintaining viability for indefinite periods with minimal genetic drift. For home cultivators without access to these technologies, the practical approach is maintaining multiple slant backups stored in separate locations, combined with periodic re-isolation from fruiting cultures to refresh genetic vitality.

## Crossbreeding and Hybridization

Creating hybrid strains through deliberate mating of compatible monokaryons is an advanced technique with significant potential. The process requires isolating monokaryotic mycelium from spore germinations, confirming mating compatibility through pairing tests, and then combining compatible monokaryons to produce dikaryotic hybrids. Stamets' observation that most hybrids underperform both parents is consistent with hybrid breeding in other organisms, where heterosis (hybrid vigor) is the exception rather than the rule. However, when successful crosses do occur, they can combine desirable traits from both parents, such as the disease resistance of one strain with the yield potential of another. Systematic breeding programs using this approach have produced commercially important mushroom varieties including several Pleurotus (oyster mushroom) hybrids used in commercial cultivation worldwide.
