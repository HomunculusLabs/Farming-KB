...
## Autoclaving and Its Limitations

Autoclaving remains the gold standard for sterilization in laboratory and
clinical settings, achieving sterility through saturated steam at temperatures
...
systems because the item must be placed inside a pressure vessel. For mycology
applications, autoclaving requires dedicated equipment, significant energy
input, and cycle times of 60 to 120 minutes, making it impractical for rapid
field sterilization of tools or surfaces between operations.

## Gamma Irradiation Constraints

Gamma irradiation from cobalt-60 or cesium-137 sources provides excellent
penetration and is effective against all microbial forms including spores.
However, the NASA documentation notes that gamma irradiation requires
specialized, expensive facilities that are not practical for routine surface
sterilization. Gamma sources must be housed in heavily shielded bunkers with
interlocked safety systems, making them accessible only at centralized
irradiation facilities. Items must be transported to these facilities, processed
in batches, and returned, introducing delays and logistical complexity.
Additionally, gamma irradiation at sterilizing doses can degrade certain
polymers, discolor materials, and alter the properties of some chemicals. For
the NASA application of sterilizing access ports between uses, shipping
components to an irradiation facility was obviously impractical. In cultivation
settings, gamma irradiation is similarly inaccessible to individual growers and
small laboratories.

## Ultraviolet Light Limitations

Ultraviolet germicidal irradiation at 254 nanometers wavelength is widely used
for surface and air disinfection. UV light damages microbial DNA, preventing
replication and leading to cell death. The NASA documentation identifies UV
