...
## Application to Cannabis Microbiome Studies

In the Winston et al. (2014) Cannabis microbiome study, 16S rRNA V4 sequencing of 69 samples (27 from the first experiment, 42 from the second) revealed:

- Significant compartment-level separation between endorhiza, rhizosphere, and bulk soil communities
- Cultivar-specific effects detectable only in the endorhiza compartment
- A conserved core [[proteobacteria-dominance-cannabis-endorhiza-community]] dominated by Pseudomonas, Cellvibrio, and Actinomycetales
- Dramatic variation in specific genera (e.g., Methylophilus) between cultivars
- The study used both weighted and unweighted UniFrac metrics, with weighted UniFrac showing stronger cultivar signals in the endorhiza

## Limitations of 16S rRNA Sequencing

Several important limitations must be considered when interpreting 16S rRNA data:

- **Taxonomic resolution**: 97% OTU clustering approximates species-level resolution but cannot reliably distinguish strains within a species. Different Cannabis cultivars may host different bacterial strains that appear identical at the OTU level.
- **Functional inference**: 16S rRNA sequencing reveals which bacteria are present but not what they are doing. Functional gene content varies substantially within OTUs.
- **PCR bias**: Universal primers do not amplify all bacterial taxa equally. Some groups are systematically underrepresented or undetected.
- **Copy number variation**: Bacterial genomes contain different numbers of 16S rRNA gene copies (1-15), and abundance estimates are confounded by this variation.
- **Dead cells**: DNA from dead or inactive bacteria is amplified alongside living cells, potentially overestimating the active community.

Despite these limitations, 16S rRNA sequencing remains the most accessible and cost-effective method for broad-scale bacterial community profiling in plant microbiome studies.
