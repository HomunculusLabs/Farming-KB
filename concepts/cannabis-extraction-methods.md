---
title: Cannabis Extraction Methods
created: 2026-04-13
updated: 2026-04-13
type: concept
tags: [cannabis, extraction, solvent, hashish, kief, essential-oil, chemistry, processing, safety]
sources: [raw/papers/ben-lemon-cannabis-alchemy.md]
---

## Cannabis Extraction Methods

Cannabis extraction concentrates the psychoactive essential oils (cannabinoids and terpenes) from plant material into more potent and refined forms. Methods range from simple mechanical separation to complex chemical processing. (see [[cervantes-hash-making-extracts]]).

### Mechanical Methods

**Kief (dry sift)**:
- Cannabis flower material is agitated over fine mesh screens
- The glandular trichome heads separate and fall through
- Different micron sizes yield different grades of purity
- Simplest and oldest method; produces a powdery concentrate
- Can be pressed into blocks with heat and pressure

**Hashish (hand-rubbed / charas)**:
- Living cannabis plants are gently hand-rubbed
- The resinous trichome material accumulates on the hands and is scraped off
- Traditional method from India, Nepal, and Morocco
- Charas from India is considered among the finest traditional hashish

**Ice water extraction (bubble hash)**:
- Plant material is agitated in ice-cold water
- Cold makes trichome heads brittle, causing them to separate
- The mixture is filtered through successively finer mesh bags (bubble bags)
- Results in graded hashish of varying purity
- Solvent-free; one of the cleanest methods available

### Solvent Extraction

Ben Lemon's "Cannabis Alchemy" provides a detailed solvent extraction protocol. The process follows these steps:

**1. Preparation**: Plant material (marijuana or hashish) is dried and ground to the finest powder possible. Ruptured cell walls allow oil to be extracted more completely. (see [[cannabis-concentrates-types]]).

**2. Reflux extraction**: The powdered material is boiled (refluxed) in solvent. The essential oil — containing THC, related cannabinoids, terpenes, chlorophyll, and flavor/smell compounds — dissolves in the solvent. Refluxing continues 3-4 hours.

**3. Solvent options** (from Lemon's guide):

| Solvent | BP | Potency | Yield | Toxicity | Notes |
|---------|-----|---------|-------|----------|-------|
| Methanol | 64°C | High | Low | High (toxic fumes) | Good selectivity; traces hazardous |
| Isopropyl (70%) | ~82°C | Lower | Higher (2x) | Moderate | Water content extracts tars |
| Ethanol | 78.5°C | High | Low | Low | Ideal but hard to obtain pure |
| Petroleum ether | 30-60°C | Highest | Low | Extreme (explosive) | Most selective; use in re-extraction |

**4. Soxhlet washing**: After reflux, the oil-bearing solvent is drained and clean solvent is refluxed through the spent material to wash out remaining oil.

**5. Solvent removal**: The solvent is distilled off using an ice-cooled condenser apparatus, leaving the essential oil behind. Pure ethanol is preferred for safety. Traces of solvent are removed by adding water and evaporating in an oil bath above the boiling point of water (100°C) but below the decomposition temperature of cannabinoids.

**6. Purification**: The crude oil is dissolved in alcohol, mixed with water, then extracted with petroleum ether. Three layers separate: bottom (water/alcohol/tars), middle (wax emulsion), top (purified cannabinoids in petroleum ether). The top layer contains the most potent fraction.

### Modern Extraction Methods

**Supercritical CO2 extraction**:
- Uses carbon dioxide at supercritical pressure/temperature as solvent
- Highly tunable: can target specific cannabinoids by adjusting parameters
- Produces clean, solvent-free extract
- Expensive equipment; industrial scale

**Butane/propane extraction (BHO)**:
- Hydrocarbon gas passed through packed column of cannabis material
- Very efficient extraction of cannabinoids and terpenes
- Produces shatter, wax, budder, and similar concentrates
- Serious safety hazard: butane is highly flammable; requires vacuum purge ovens

**Rosin press**:
- Heat and pressure applied to cannabis flower or hash
- Mechanical extraction; no solvents required
- Produces high-quality, solventless concentrate
- Increasingly popular for home use

### Safety Considerations

- All solvent-based methods involve flammable and/or toxic materials
- Proper ventilation, no open flames, and fire suppression equipment are essential
- Residual solvent in the final product is a health concern — proper purging is critical
- The ice-water condenser method described by Lemon is specifically designed to prevent solvent vapor from reaching the air

## See Also

- [[cannabis-plant-anatomy]]
- [[cannabis-genetics-and-breeding]]
- [[cannabis-extraction-basics]]
