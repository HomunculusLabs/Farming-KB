---
title: Law of Return and Energy Catchment Design
...
Water flowing from hills to a valley lake demonstrates the principle concretely. If no attempt is made to store or use water as it passes through a system, the opportunity is lost entirely. The permaculture approach designs multiple interception points:

- **Swales and contour banks** to slow and spread water across the landscape
- **Dams and ponds** at multiple elevations to store water at the highest possible points
- **Mulch and organic matter** in soils to hold water in the root zone
- **Forest canopy** to intercept rainfall, reduce evaporation, and slow runoff
- **Roof catchment** for domestic water supply

Each interception point catches energy (in this case, water with its potential and chemical energy) before it reaches the sink where it becomes unavailable for useful work.

## Energy Types in the Design Net
...
Energies enter a system and either remain or escape. The permaculture designer's work is to prevent energy leaving before the basic needs of the whole system are satisfied, so that growth, reproduction, and maintenance continue in all living components. This means designing for:

- **Multiple uses** of each energy flow (stacking functions)
- **Highest possible storage** at each interception point
- **Longest possible residence time** of energy within the system
- **Maximum number of passes** through different biological components before energy reaches its sink

The design goal is not to stop entropy — this is impossible — but to extract maximum useful work from every joule of energy that enters the system before it degrades to a state where it is no longer useful.
