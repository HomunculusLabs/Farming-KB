...
- **Ground cover**: Clover or nasturtiums — fix nitrogen, suppress grass, attract beneficial insects
...
For those lacking time for extended observation, guilds can be designed using plant reference books and ecological knowledge. The process involves: (1) selecting a central element, (2) identifying the environmental conditions it creates (shade, root competition, allelopathy), (3) choosing nitrogen fixers compatible with those conditions, (4) adding insectary plants, (5) including mulch makers and dynamic accumulators, (6) selecting ground covers, and (7) filling remaining niches with food-producing or habitat species. Plant identification guides, ecology texts, and permaculture plant databases provide the necessary species information.

## Guild Design Principles

1. **Each element performs multiple functions** — a comfrey plant accumulates minerals, produces mulch, attracts bees, and suppresses weeds with its large leaves.
2. **Each function is supported by multiple elements** — pest control comes from insectary plants, bird habitat, and biological sprays, not just one mechanism.
3. **Maximize beneficial connections** — design for the relationships between plants, not just the plants themselves.
4. **Use edge and diversity** — the boundaries between guild members are where the most ecological activity occurs.
5. **Observe before designing** — the most successful guilds are based on prolonged observation of natural communities.
6. **Start simple, add complexity** — begin with three to five species and add more as you observe what works.

## Guilds in Broader Landscape Design

Individual guilds are building blocks for larger landscape patterns. Multiple guilds can be connected to create food forests, edible landscapes, and integrated homestead designs. Guilds also serve as educational tools — studying a single guild reveals principles that scale to whole-property design.

### Guilds for Specific Climates
...
- **Temperature range**: Frost tolerance determines which species can overwinter. Tropical guilds include bananas, coffee, and cacao, while temperate guilds use apples, pears, and stone fruits.
...
- **Guilds** are defined by their interconnected functions — each species supports the others through specific ecological services (nitrogen fixation, pest control, mulch production). The emphasis is on the web of relationships.
